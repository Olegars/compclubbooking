...
watch(() => [props.rulesComplete, onReview.value], () => {
    ...
    if (!props.rulesComplete) rulesOpen.value = true
}, { immediate: true })
...
const canHire = computed(() => canEdit.value && props.rulesComplete && fieldsFilled.value && !form.processing)
...
<template>
    <div class="space-y-8">
        ...
        <div v-if="onReview" class="bg-[#0a0a0a] border border-amber-500/30 rounded-[1.125rem] p-10 text-center">
        ...
        <template v-else>
            <div v-if="isRejected" class="bg-red-500/10 border border-red-500/30 rounded-[1.125rem] p-6">
                <div class="text-[10px] uppercase font-black tracking-widest text-red-400">Отклонено</div>
                <p class="text-white text-sm font-bold mt-3 whitespace-pre-wrap">{{ rejectionReason || 'Анкета отклонена.' }}</p>
                <p class="text-white/40 text-[11px] font-bold mt-3">Исправьте данные и отправьте анкету снова.</p>
            </div>
            ...
            <form class="bg-[#0a0a0a] border border-white/5 rounded-[1.125rem] p-8 space-y-6"
                ...
                <button v-if="canHire" type="submit"
                    ...
                    {{ form.processing ? 'Отправка…' : (isRejected ? 'Отправить снова' : 'Устроиться') }}
                </button>
                <div v-else class="text-[10px] uppercase font-black tracking-widest text-white/25 text-center py-4">
                    Кнопка появится, когда приняты все правила и заполнены данные
                </div>
            </form>
        </template>
    </div>
