        ...
        <div class="cabinet max-w-7xl mx-auto w-full font-mono pb-16 sm:pb-20 text-white animate-in fade-in duration-700">
            ...
            <div class="md:col-span-2 min-w-0 flex flex-col gap-3 md:gap-6 px-4 md:px-0 bg-transparent">
                ...
                <div class="cabinet-block min-w-0 bg-white/5 md:bg-[#0a0a0a] border border-white/10 md:border-[#22c55e]/20 rounded-xl md:rounded-[1.125rem] p-4 sm:p-8 md:p-10 relative md:shadow-2xl md:shadow-[#22c55e]/5">
                    ...
                    <div class="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-5 lg:gap-8 items-start">
                        <div class="min-w-0">
                            <span class="text-[10px] uppercase text-[#22c55e] tracking-[0.2em] sm:tracking-[0.35em] font-black italic break-words pr-1">Баланс {{ userName }}</span>
                            <div class="mt-2 sm:mt-4 flex items-baseline gap-2 sm:gap-4">
                        ...
                        <div v-if="featureOn('arena_duels')" class="min-w-0 lg:pl-6 lg:border-l lg:border-orange-400/20">
                            ...
                            <div class="grid grid-cols-2 gap-2 mb-2">
                                <button type="button" @click="arenaForm.kind = 'duel'"
                                        :class="['py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest', arenaForm.kind === 'duel' ? 'bg-orange-400 text-black' : 'bg-black/40 border border-white/10 text-white/50']">
                                    Дуэль
                                </button>
                                <button type="button" @click="arenaForm.kind = 'battle'"
                                        :class="['py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest', arenaForm.kind === 'battle' ? 'bg-orange-400 text-black' : 'bg-black/40 border border-white/10 text-white/50']">
                                    Битва
                                </button>
                            </div>
                    ...
                    <div v-if="featureOn('arena_duels') && (arenaMapReady || arenaMine || arenaIncoming || arenaBoard.length || (arenaData.live || []).length)"
                        ...
                        <div v-for="row in (arenaData.live || [])" :key="'live-'+row.uuid" class="text-[11px] text-white/60 mt-2">{{ row.line }}</div>
                    </div>
                </div>
            ...
            <div class="min-w-0 flex flex-col gap-3 md:gap-6 md:space-y-0 bg-transparent md:sticky md:top-28 md:self-start mt-3 md:mt-0 px-4 md:px-0 overflow-visible">
                ...
                <div v-if="featureOn('achievements') || achievements.length > 0" class="cabinet-block bg-white/5 md:bg-[#0a0a0a] border border-white/10 md:border-purple-500/20 rounded-xl md:rounded-[1.125rem] p-4 sm:p-6 md:p-8 md:shadow-xl">
                    ...
                    <div v-if="achievements.length > 0" class="space-y-3 sm:space-y-4">
                    ...
                    <div v-else class="text-center py-6 text-white/20 italic uppercase text-[10px] tracking-widest border border-dashed border-white/10 rounded-xl">
                        Квесты пока не назначены
                    </div>
                </div>
