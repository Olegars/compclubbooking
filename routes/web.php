...
Route::get('/admin-app.apk', function () {
    $path = storage_path('app/apk/admin0451.apk');
    abort_unless(is_file($path), 404);

    $response = response()->file($path);
    $response->headers->set('Content-Type', 'application/vnd.android.package-archive');
    $response->headers->set('Content-Disposition', 'attachment; filename="admin0451.apk"');
    $response->headers->set('X-Content-Type-Options', 'nosniff');
    $response->headers->set('Cache-Control', 'private, no-transform');

    return $response;
})->name('admin-app.apk');
Route::get('/admin-app.json', function () {
    $path = storage_path('app/apk/admin0451.apk');
    abort_unless(is_file($path), 404);

    return response()->json([
        'version_code' => (int) config('admin_app.version_code'),
        'version_name' => (string) config('admin_app.version_name'),
        'apk_url' => url('/admin-app.apk'),
        'size' => filesize($path),
    ]);
})->name('admin-app.json');
...
Route::get('/boss-app.apk', function () {
    $path = storage_path('app/apk/boss0451.apk');
    abort_unless(is_file($path), 404);

    $response = response()->file($path);
    $response->headers->set('Content-Type', 'application/vnd.android.package-archive');
    $response->headers->set('Content-Disposition', 'attachment; filename="boss0451.apk"');
    $response->headers->set('X-Content-Type-Options', 'nosniff');
    $response->headers->set('Cache-Control', 'private, no-transform');

    return $response;
})->name('boss-app.apk');
Route::get('/boss-app.json', function () {
    $path = storage_path('app/apk/boss0451.apk');
    abort_unless(is_file($path), 404);

    return response()->json([
        'version_code' => (int) config('boss_app.version_code'),
        'version_name' => (string) config('boss_app.version_name'),
        'apk_url' => url('/boss-app.apk'),
        'size' => filesize($path),
    ]);
})->name('boss-app.json');
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::prefix('account')->group(function () {
        ...
        Route::get('/arena/live', [ProfileController::class, 'arenaLive']);
        Route::post('/arena/challenges', [ProfileController::class, 'createArena']);
...
Route::middleware(['auth:admin', 'staff.active'])->prefix('admin')->group(function () {
    ...
    Route::middleware(['role:store_manager,assembler,senior_manager,owner', 'store'])
        ...
        ->group(function () {
            ...
            Route::post('/avito/chats/sync', [StoreAvitoController::class, 'syncChats'])->name('admin.store.avito.chats.sync');
            Route::post('/avito/chats/read-all', [StoreAvitoController::class, 'markAllRead'])->name('admin.store.avito.chats.read-all');
    ...
    Route::middleware(['role:supervisor,owner'])->group(function () {
        ...
        Route::post('/api/arena/duels/{id}/force-refund', [ArenaDuelAdminController::class, 'forceRefund']);
...
Route::prefix('api/shell')->group(function () {
    ...
    Route::get('/arena/live', [ShellLanLiveController::class, 'arenaLive']);
    Route::post('/arena/challenges', [ShellLanLiveController::class, 'createArena']);
