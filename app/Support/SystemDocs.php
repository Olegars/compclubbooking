            ...
            [
                ...
                'items' => [
                    ...
                    [
                        ...
                        'description' => "Редактирование никнейма, email и фото (ЛК /account/profile и карточка аватара в кабинете). Телефон зафиксирован как идентификатор входа. Стартовый ник при первой регистрации выдаёт DeepSeek (см. «Игровой ник»).\n\nСвоё фото: POST /account/profile/avatar, JPEG/PNG/WebP/GIF до 8 МБ, кроп в квадрат 512 px, файл в storage/avatars, users.avatar = custom/u{id}_….png. Галка «Стилизовать под клубный формат» шлёт снимок и образец из public/images/avatars/avatar_*.png в DeepSeek. Хостовый API не рисует PNG — просим SVG и растеризуем (Imagick/rsvg). Если модель вернула только текст, фото само красится в клубной гамме (тёмный круг, неон, схемы); чужой дефолтный портрет поверх не кладётся. Если в админке есть OpenAI-ключ — дополнительно images/edits. Без галки фото сохраняется как есть.",
