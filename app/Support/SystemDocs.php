        ...
        return [
            ...
            [
                ...
                'items' => [
                    ...
                    [
                        'title' => 'LAN Arena (котлы дуэлей / Skill Pot)',
                        'description' => "Открытый мини-конкурс мастерства (ГК РФ ст. 1057–1061), не пари и не касса 244-ФЗ. Взнос списывается с депозита в эскроу; приз возвращается на депозит клуба (время / бар / REACTOR Store), без вывода на карту. Сбор клуба (rake) — организационный доход в момент закрытия матча, без нового чека полного расчёта: аванс уже фискализован при пополнении.\n\nДва режима: дуэль (двое, старт после акцепта) и битва (3–N, лобби открыто до старта / набора / времени). Вызов из шелла или заранее из ЛК без активной сессии. Открытые вызовы всем желающим — карточки в ЛК «Арена / дуэли» (время, игра, ставка, категория). Принять списывает взнос. Уже принявшие могут предложить повысить ставку: применяется, только если все в лобби согласны (доплата дельты с депозита).\n\nФича /admin/config/features → «Арена дуэлей»: мин/макс взнос, % сбора, TTL адресного и открытого котла, TTL вызова из ЛК, мин/макс битвы, допуск ранга LFG, таймаут дисконнекта, кулдаун спама, first-to CS2, ваучер на кухню.\n\nШелл: вкладка «АРЕНА», POST /api/shell/arena/challenges {kind: duel|battle, game, mode: 1v1_aim|2v2_wingman|1v1_mid, entry_fee, scope: hall|computer|zone, target_computer_id, scheduled_at, max_players}. Акцепт/отклонение/отмена/старт/рейз: /accept /decline /cancel /start /raise /raise-vote. Доска GET /api/shell/arena/live и lan-live/balance. GSI тот же POST /api/shell/gsi (алиас /api/shell/arena/gsi): round_win до first_to или match_win при живом GSI соперника. 2v2 — пати из ровно двух активных ПК, приз делится. Ранг: последний LFG той же игры, неизвестный не блокирует. Дисконнект 90 с (GSI или last_seen) при живом оппоненте — техническое поражение. Админ: POST /admin/api/arena/duels/{id}/force-refund (supervisor+).\n\nЛК: GET /account/arena/live, POST /account/arena/challenges те же действия без обязательной сессии на ПК. Карточки hall, входящий адресный вызов, повышение ставки. TV: arena_duel в GET /api/shell/overlays.\nКод: ArenaDuelService. Тесты: ArenaDuelsTest.",
                        'path' => '/admin/config/features',
