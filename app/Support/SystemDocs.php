            ...
            [
                ...
                'items' => [
                    ...
                    [
                        ...
                        'description' => "Наклейка на корпусе проданного ПК: QR → GET /pc/{token} (не серийник — токен 32 символа). Мобильная страница без входа: кто собирал (имя), таймлайн (старт сборки, сверка серийников, готов, выдача, ремонт), серийники деталей и остаток гарантии каждой онлайн.\n\nВидео сборки — камера стола сборщика через NVR. Заказ → assembling ставит метку store.assembly_start; сверка/ready — store.assembly_done и очередь store_assembly_clip_jobs. Тот же LAN-агент scripts/hikvision-marker-agent.ps1 (нужен ffmpeg) pull GET /api/video/assembly-clip-targets, режет интервал с NVR (RTSP playback) и POST /api/video/assembly-clips. Канал: STORE_ASSEMBLY_NVR_CHANNEL или канал события / default_channel. Максимум минут: STORE_ASSEMBLY_CLIP_MAX_MINUTES (20). Пока ролик не выгрузился — на паспорте текст ожидания. Ручная загрузка mp4 с карточки сборки. Телефон клиента и полное ФИО на странице не показываются.",
                        'path' => '/admin/store/built-pcs',
            ...
            [
                ...
                'items' => [
                    ...
                    [
                        ...
                        'description' => "Суммирование побед и очков CS2/Dota GSI между сторонами в реальном времени.\n\nРежимы. Зоны одной локации: Bootcamp (bootcamp / bootcamp-pro) vs Standard (сингл/дуо/трио/кватро). Локации сети: Club A vs Club B (clubs).\n\nАдмин: /admin/clan-wars (Киберспорт) — создать, «В эфир», завершить. Одна live-война. GSI match_win = 10 очков + победа, round_win = 1 очко. Дедуп по игроку/матчу/раунду. По окончании Elo-рейтинг фракции и личный вклад в ЛК.\n\nTV/PC lobby: GET /api/shell/overlays отдаёт clan_war и рисует счёт текстом на DAT mid_left / mid_right. Idle TV поллит быстрее, пока война live. GET /api/shell/clan-wars/live без сессии. Сессионный шелл видит clan_war в lan-live / balance / gsi.\n\nЛК: блок Clan Wars — live-счёт, свой рейтинг, таблица кланов.\nКод: ClanWarService. Тесты: ClanWarsTest.",
