            ...
            [
                ...
                'items' => [
                    ...
                    [
                        ...
                        'description' => "Сейчас в проде: квесты play_hours / night_visits / visit_count, награда депозит или фантики после закрытия сессии (/admin/achievements).\n\nПлан — расширить в хаб профиля: аватары и рамки, клубные статусы, значки ивентов, боевой пропуск (XP → реальные награды), лестница скидок/кешбэка часами, каталог ачивок из GSI / Steam / FACEIT / OpenDota / Riot / PUBG / Tracker / биллинга / арены. Полное ТЗ — раздел «Профиль, ачивки и боевой пропуск».",
                    ...
                    [
                        'title' => 'Профиль, ачивки и боевой пропуск (план)',
                        'description' => "Витрина гостя (рамка, статус, значки), XP-пропуск с реальными наградами, каталог GSI/Steam/OpenDota/Riot/бар/арена. Админка — вкладки на /admin/achievements, не новый пункт сайдбара. Полное ТЗ — раздел «Профиль, ачивки и боевой пропуск».",
                        'path' => '/admin/docs',
                        'audience' => 'Supervisor+ / Игрок / Shell',
                    ],
                ],
            ...
            [
                ...
                'items' => [
                    [
                        ...
                        'audience' => 'Owner / Игрок / Shell',
                    ],
                    [
                        'title' => 'Личный Steam, не пул клуба',
                        'description' => "FACEIT CS2 привязан к личному Steam игрока (games.cs2.game_player_id = SteamID64). Официальный MM не примет клубный game_accounts из /admin/licenses: это чужой Steam, ToS FACEIT запрещает шаринг, VAC/AC увидят карусель логинов на одном железе.\n\nКанон запуска с зала.\n1) Сессия шелла active. Фича faceit on.\n2) В login/balance — блок faceit: linked, nickname, skill_level, elo, banned, steam64.\n3) Шелл не вызывает take аккаунта Steam клуба на этот запуск. CS2 стартует в библиотеке гостя (свой Steam / Family Sharing на этом ПК).\n4) Клиент FACEIT — свой логин. После OAuth облако не подставляет пароль FACEIT: SSO в клиент через API нельзя.\n5) Нет привязки — шелл: «Привяжи FACEIT в ЛК» + QR на /account/profile, кнопку очереди не красим как готов.\n\nЕсли гость хочет просто «поиграть CS2» без FACEIT — обычный путь лицензий клуба, как сейчас.\nПул Steam остаётся для оффлайн/премьера/казино-режимов, не для FACEIT MM.\nGSI steam_id (охота / трон) можно предложить как подсказку к привязке GET /players?game=cs2&game_player_id={steam64}, не как автологин.",
                        'path' => '/admin/licenses',
                        'audience' => 'Игрок / Shell / Supervisor+',
                    ],
                    [
                        'title' => 'Клиент и AC на бездиске',
                        'description' => "Железо. Клиент и kernel FACEIT AC (FACEIT.sys) живут на C: золотого образа (Super Client, диск image). Не ставить AC в writeback гостя: после reboot пропадёт, на 40 ПК поедет разный билд, HWID-сервис сломается. AC проверяет целостность ядра до сессии и требует TPM 2.0 + Secure Boot в UEFI каждой платы зала — без этого служба не поднимется на бездиске. Регресс: после save образ грузится на втором ПК, FACEIT AC в services.msc Running до логина гостя.\n\nЛовушка Super Client. FACEIT.sys — Boot-start: поднимается до сессии. Update клиента в Super Client и сразу Save Image без чистого reboot эталона — служба зависает на клиентах, отказ инициализации / «The service cannot be started».
Регламент фазы 0 (все платы: TPM 2.0 + Secure Boot On). На эталоне Super Client: установка/Update → чистый Reboot (ПК ещё SC) → sc query FACEIT статус RUNNING → Shutdown → сохранение образа в панели бездиска. Не Save с живого полуобновлённого сеанса и не Disable Super Client до проверки sc query.\n\nПрофиль целиком, не один Local. Electron-клиент пишет кэш и куки в %LOCALAPPDATA%\\FACEIT и %APPDATA%\\FACEIT (Roaming), плюс HKCU\\Software\\FACEIT. Junction только Local оставляет токены в Roaming/реестре — следующий гость подхватит чужой логин. На login шелла:\n• D:/ShellData/faceit/{user_id}/Local → %LOCALAPPDATA%\\FACEIT\n• D:/ShellData/faceit/{user_id}/Roaming → %APPDATA%\\FACEIT\nНе класть FACEIT на C:\\Users\\user\\AppData как в сиде quick-apps — путь завязан на учётку образа.\n\nLogout / idle shutdown (обязательный набор, не «убить окно»).
1) taskkill /F /IM FACEIT.exe /T (дерево Electron).
2) reg delete \"HKCU\\Software\\FACEIT\" /f (machine-guid и флаги автологина; иначе общий профиль C:).
3) Стереть D:/ShellData/faceit/{user_id} (Local и Roaming).
4) Снять оба junction.
Службу FACEIT.sys не останавливать (образ). Следующий гость не залогинен чужим ником.\n\nКонфликт с REACTOR AC. На клубном ПК гость в FACEIT MM использует FACEIT AC; REACTOR AC на зал не ставится (шелл = station_trusted на дедик клуба). На домашнем ПК оба клиента не патчить — глава античита «Право, подпись, конфликт с чужими AC».\n\nКиоск. FACEIT и AC — доверенные в SecurityManager (как Steam/Discord). Не Disable для их служб. Win+R по-прежнему закрыт.\nПодробности рисков — глава «Критические риски».",
                        'path' => null,
                        'audience' => 'Техник / Shell',
                    ],
                    [
                        'title' => 'FACEIT Connect: привязка аккаунта',
                        'description' => "Как «Привязать Telegram», только OAuth2 FACEIT Connect 3.0 (authorization code, секрет на сервере). Не ник руками и не скрейп HTML. OAuth нужен только один раз: подтвердить владение через GET /auth/v1/userinfo (faceit_player_id + steam_id_64). Elo CS2 дальше читает серверный FACEIT_API_KEY, не Bearer игрока.\n\nПоток.\n1) ЛК /account/profile — «Привязать FACEIT». GET /account/faceit/redirect → authorize scope=openid (openid_configuration https://api.faceit.com/auth/v1/openid_configuration). state в сессии, PKCE если App Studio требует.\n2) Callback GET /account/faceit/callback?code=&state=.\n3) Облако меняет code (client_secret только .env) и один раз GET https://api.faceit.com/auth/v1/userinfo.\n4) Сразу Data API ключом GET /players/{id} — ник, аватар, skill_level, elo. User Bearer дальше не нужен.\n5) Upsert faceit_identities (unique faceit_player_id, unique user_id). Чужой клубный аккаунт с тем же player_id — отказ, как у telegram_chat_id.\n6) Токены выкинуть. Unlink POST /account/faceit/unlink: строки identities нет. refresh_token и access_token в БД не хранить: клуб ничего не пишет от лица гостя.\nСхема целиком — глава «Поток данных».\n\nScope: openid достаточно. Не просить chat, offline_access, запись профиля. Не чат-бот.\n\nRedirect URI в App Studio строго один URL, без wildcard и без localhost на проде: https://{APP_HOST}/account/faceit/callback. Смена домена = правка клиента в App Studio (или второй OAuth-клиент). Несколько брендов/хостов — отдельные client_id или один канонический APP_URL.\n\nШелл OAuth не открывает: киоск съест попап FACEIT. Привязка с телефона в ЛК, на ПК только статус.",
                        'path' => '/account/profile',
                        'audience' => 'Игрок / Система',
                    ],
                    [
                        'title' => 'Поток данных: ЛК → API → шелл',
                        'description' => "Канон фазы 1. Шелл и Data API не ходят друг к другу: шелл только poll клуба.\n\nГость в ЛК с телефона\n1) OAuth2 Connect 3.0 (scope=openid) → api.faceit.com\n2) Callback GET /account/faceit/callback?code=… → Laravel\n3) Обмен code, GET /auth/v1/userinfo → faceit_player_id + steam_id_64. Токены в RAM, в БД не писать.\n4) Data API v4 Authorization: Bearer FACEIT_API_KEY → GET /players/{id} (и при синке bans/stats)\n5) Upsert faceit_identities: elo, skill_level, steam_id_64, nickname. Колонок access_token / refresh_token нет.\n\nШелл ПК в зале (VLAN 20)\n• Опрос /api/shell/balance (и login / lan-live) → блок faceit {linked, elo, skill_level, nickname, banned, steam64}\n• Запуск: личный Steam гостя, никакого take() из /admin/licenses\n• На login: junction %LOCALAPPDATA%\\FACEIT → D:/ShellData/faceit/{user_id}/Local и %APPDATA%\\FACEIT → …/Roaming\n• На logout: taskkill /F /IM FACEIT.exe /T, затем reg delete \"HKCU\\Software\\FACEIT\" /f, wipe D:, снять junction\n\nНе смешивать. userinfo — identity. GET /players/{id} — публичная статистика ключом клуба. Клиент FACEIT на ПК — свой логин гостя, облако пароль не подставляет.",
                        'path' => '/account/profile',
                        'audience' => 'Система / Shell',
                    ],
                    [
                        'title' => 'Data API v4: Elo, баны, кэш',
                        'description' => "Серверный ключ App Studio (Server side), заголовок Authorization: Bearer {FACEIT_API_KEY}. База https://open.faceit.com/data/v4. Ключ не в шелл и не в фронт.\n\nЧтение (публичные данные).\n• GET /players?nickname= или GET /players?game=cs2&game_player_id={steam64} — резолв до player_id.\n• GET /players/{player_id} — ник, аватар, steam_id_64, games.cs2 {skill_level 1–10, faceit_elo, game_player_id}.\n• GET /players/{player_id}/stats/cs2 и /games/cs2/stats — K/D, винрейт, последние карты (карточка ЛК, не для LFG).\n• GET /players/{player_id}/bans — если бан активен, шелл: «FACEIT бан», очередь не предлагать.\n• GET /players/{player_id}/history — опционально лента в ЛК.\n• Хаб: GET /hubs/{hub_id}, /matches, /members, /leaderboards/hubs/{hub_id}.\n• Матч: GET /matches/{match_id}, /stats — после webhook.\n\nКэш. Таблица faceit_identities: elo, skill_level, banned_until, synced_at. Сидящие в зале (active booking) — reactor:sync-faceit каждые 10–15 мин. Остальные — 12–24 ч или после unlink/login. 429 Too Many Requests — backoff, не долбить с каждого poll шелла (~8 с). Шелл читает только облако, open.faceit.com с VLAN 20 не обязан (клиент FACEIT сам ходит на свои хосты).\nBearer игрока для Elo не нужен: Data API — FACEIT_API_KEY. User-token после Connect не оставлять.\n\nЗапись матчей/очереди Data API не даёт. Создание хаба/чемпионата — руками на faceit.com под организатором, в админку кладём UUID.",
                        'path' => '/admin/config/features',
                        'audience' => 'Система',
                    ],
                    [
                        'title' => 'ЛК, шелл, LFG, стек',
                        'description' => "ЛК /account/profile и кабинет: карточка FACEIT (аватар платформы, ник, уровень 1–10, Elo CS2, бан, «Отвязать»). Нет ключа / фича off — блока нет. Не подменять users.name ником FACEIT: клубный ник остаётся из PlayerNicknameService.\n\nШелл. login / balance / lan-live: faceit {linked, nickname, skill_level, elo, banned}. Быстрые приложения: FACEIT из каталога после junction Local+Roaming. Пока not linked — бейдж, не автозапуск.\n\nLFG CS2. Если linked и не banned — rank_tier = skill_level (1–10), строку «faceit» руками не просить. Нет привязки — как сейчас (silver/global/заглушка). rank_delta из фичи lfg. Не матчить FACEIT-10 с FACEIT-2 из-за одной заглушки «faceit».\n\nСтек 5. Пати в зале (BookingGroup) + все linked — карточка «Стек FACEIT: ПК-01…05, Elo ~2100». Игроки жмут очередь в клиенте FACEIT сами. Облако не вызывает matchmaking API (его нет для приложения клуба). Голос — игровой / Discord клуба, не Chat API FACEIT в MVP.\n\nАрена дуэлей клуба — разминка до FACEIT, GSI, без денег. Не путать с хабом: арена на дедике клуба (REACTOR AC / общий пароль пока gate off), хаб — сервера FACEIT.",
                        'path' => '/account/dashboard',
                        'audience' => 'Игрок / Shell',
                    ],
                    [
                        'title' => 'Хаб клуба и webhooks',
                        'description' => "Фаза 2. Организатор создаёт Hub на faceit.com (CS2, регион EU/RU как у зала, AC required). UUID хаба — FACEIT_HUB_ID в .env / club_features. Игроки вступают в хаб сами (ссылка в ЛК «Вступить в хаб клуба», faceit_url из GET /hubs/{id}). Клуб через Data API не добавляет членов без Organizer/Chat API и партнёрки; MVP = ссылка + сверка GET /hubs/{id}/members.\n\nApp Studio → Webhooks, подписка организатора. POST /api/faceit/webhook (заголовок FACEIT_WEBHOOK_SECRET, как TELEGRAM_WEBHOOK_SECRET). События: match_object_created, match_status_ready, match_status_finished, match_status_aborted/cancelled, match_demo_ready, hub_user_added/removed. Идемпотентность по event_id + match_id.\n\nready: если в матче есть faceit_player_id сидящего в зале — TV/оверлей faceit_match (карта, фракции, Elo), слот не конфликтует с live Clan Wars (приоритет: clan_war > faceit_hub > arena_duel, либо отдельный DAT).\nfinished: upsert faceit_matches, обновить Elo участников через GET /players/{id}. Не начислять депозит и не трогать Event Manager prizes_paid_at — призы хаба на стороне FACEIT, если включены там.\n\nChat API (бот в лобби матча) — не MVP: нужен user-token, не server key.\nGame Server SDK / Facebot — не цель: клуб не регистрирует свой дедик как FACEIT GS.",
                        'path' => '/admin/overlays',
                        'audience' => 'Supervisor+ / Система / Shell',
                    ],
                    [
                        'title' => 'Чемпионат vs Менеджер ивентов',
                        'description' => "Два контура, не сливать в одну таблицу tournaments.\n\nEvent Manager (/admin/tournaments). Межклубный вызов, сетка Single Elim, счёт вручную, призы на депозит, lock_games на ПК арены. Игры любые. FACEIT в поле rules сейчас — свободный текст («BO3, FACEIT»). Так и оставить для лора, пока нет championship_id.\n\nFACEIT Championship. Организатор создаёт чемпионат на faceit.com. В админке фаза 3: опционально championship_id на карточке локального ивента → read-only сетка/результаты GET /championships/{id}/matches|/results, без ручного счёта. Призы FACEIT ≠ «Завершить + призы» клуба: не двойная выплата. lock_games можно включить на CS2, пока чемпионат ongoing.\n\nНе обещать автосоздание чемпионата из «Вызвать клуб»: Organizer write API не в публичном Data API. Пока UUID копируют руками.\n\nClan Wars / арена / трон ПК — GSI зала, к FACEIT не привязаны.",
                        'path' => '/admin/tournaments',
                        'audience' => 'Supervisor+',
                    ],
                    [
                        'title' => 'Критические риски: NAT-бан, TPM, профиль, OAuth',
                        'description' => "Закрыть в фазе 0 / Connect, не оставлять на «потом». Иначе один читер или кривой save образа валит весь зал.\n\n1) Бан подсети (Ban Evasion / Multi-accounting). Десятки гостей выходят в FACEIT с одного внешнего IP зала (NAT). Бан одного за читы или смурф — алгоритмы FACEIT могут временно или навсегда закрыть всю публичную подсеть. Баны «паровозиком» ни в чём не повинным.\nКупирование. До открытия FACEIT на полу: официальный тикет в саппорт FACEIT на LAN Center / Cyber Cafe IP Whitelist (статический egress VLAN 20). На RB5009 — выделенный Source NAT: игровой VLAN 20 на отдельный статический белый IP; VLAN 40 hotspot и бар/касса не должны делить этот egress. Не masquerade «весь LAN одним IP». Пока whitelist не подтверждён — фича faceit off, ярлык можно не светить как «готов к рейтингу».\n\n2) TPM, Secure Boot, FACEIT.sys в Super Client. FACEIT AC требует TPM 2.0 и Secure Boot. Драйвер FACEIT.sys проверяет целостность ядра до сессии. Бездисковая загрузка без TPM/SB = служба не стартует на всех 40 ПК.\nЛовушка: Update клиента в Super Client → сразу Save Image без чистого reboot → «The service cannot be started».\nРегламент. UEFI 100% плат: TPM 2.0 On, Secure Boot On (если SB ломает PXE — не включать FACEIT AC, пока связка не проверена на одном ПК). На Super Client: установка/Update → чистый Reboot эталона → sc query FACEIT = RUNNING → Shutdown → сохранение образа в панели бездиска. Не Save Image до reboot.\n\n3) Профиль Local + Roaming + HKCU. Junction только %LOCALAPPDATA%\\FACEIT оставляет куки Electron в %APPDATA%\\FACEIT и HKCU\\Software\\FACEIT. Logout без wipe реестра = утечка связки логинов.\nКанон. Два junction на D:/ShellData/faceit/{user_id}/Local и /Roaming. Logout: taskkill /F /IM FACEIT.exe /T, затем reg delete \"HKCU\\Software\\FACEIT\" /f, wipe папки на D:, снять junction. Службу FACEIT.sys не трогать.\n\n4) OAuth: openid и один Redirect URI. App Studio не принимает wildcard и localhost на проде; URI = канон https://{APP_HOST}/account/faceit/callback. Elo CS2 — FACEIT_API_KEY, не user Bearer. Скоуп openid достаточно для identity (player_id + SteamID). refresh_token / access_token в БД не хранить: клуб не пишет от лица гостя.\n\nНе успех фазы 0. Общий NAT зала+Wi-Fi. Нет тикета whitelist. Save образа AC без reboot. Один junction Local. Токены гостя в faceit_identities.",
                        'path' => '/admin/docs',
                        'audience' => 'Owner / Техник / Система',
                    ],
                    [
                        'title' => 'Что не цель и прочие слепые зоны',
                        'description' => "Не цель.\n• Свой дедик = сервер FACEIT (Labs Game Server SDK).\n• Поставить 10 гостей в очередь MM одним POST из booking.\n• Логин FACEIT паролем из облака / автозаполнение клиента.\n• Клубный Steam take() на матч FACEIT.\n• Скрейп faceit.com вместо Data API.\n• Premium / подписка клуба на всех гостей.\n• Патч AC, отключение AC «чтобы бездиск не тормозил».\n• Elo FACEIT как тариф зала или допуск в зону Bootcamp (отдельное продуктовое решение, не в MVP).\n\nСлепые зоны (продукт; сеть/TPM/профиль/OAuth — глава «Критические риски»).\n• Гость логинится в FACEIT, но в CS2 — клубный Steam: AC/очередь отвалятся. Шелл предупреждает, take Steam на этот запуск не делает; принудительно выкинуть из чужого Steam API Steam не даёт — инструкция в UI.\n• Профиль не уехал на D: целиком (Local+Roaming) или HKCU жив после logout — чужой логин. Приёмка фазы 0.\n• Два гостя, один player_id — unique constraint, второй unlink у первого не делает молча.\n• Webhook без секрета — не принимать. Повтор finished — идемпотентный upsert.\n• 429 API — карточка «обновлено N мин назад», не пустой Elo.\n• Хаб пустой: никто не вступил по ссылке — это не баг синка members.",
                        'path' => '/admin/docs',
                        'audience' => 'Owner / Система',
                    ],
                    [
                        'title' => 'API, таблицы, фича, env',
                        'description' => "Фича club_features key=faceit mode=off|identity|hub (как reactor_ac: пустой club_features в v1 не включает FACEIT, пока ключей нет). identity = OAuth+Elo+LFG+шелл. hub = +webhook+оверлей. Поля: hub_id, organizer_id, game_id=cs2, sync_seated_minutes=15.\n\nEnv облака (config/services.php лицо faceit):\n• FACEIT_API_KEY — Data API server key;\n• FACEIT_CLIENT_ID / FACEIT_CLIENT_SECRET — OAuth приложения App Studio;\n• FACEIT_REDIRECT_URI — канон callback;\n• FACEIT_WEBHOOK_SECRET — заголовок входящего POST;\n• FACEIT_HUB_ID / FACEIT_ORGANIZER_ID — опционально, дубль в фиче клуба.\nКлючи не в Qt config.ini.\n\nТаблицы.\n• faceit_identities: user_id unique, faceit_player_id unique, nickname, avatar_url, steam_id_64, skill_level, elo, game_id, banned_until, synced_at. Колонок access_token / refresh_token нет: identity одноразовая, Elo — FACEIT_API_KEY.\n• faceit_matches: match_id unique, hub_id, championship_id, status, map, started_at, finished_at, payload json, demo_url.\n• faceit_match_players: match_id, faceit_player_id, user_id nullable, faction, elo_before/after.\nНе писать токены FACEIT в users.*; telegram_* — соседний паттерн, не смешивать колонки. refresh_token гостя в БД не хранить.\n\nAPI клуба.\n• GET /account/faceit/redirect, GET /account/faceit/callback, POST /account/faceit/unlink.\n• POST /api/faceit/webhook (без сессии игрока, секрет).\n• GET /api/shell/balance и lan-live — вложенный faceit.\n• artisan reactor:sync-faceit — кэш Elo сидящих + просроченный кэш.\nАдминка фаза 2: /admin/faceit (статус ключа, хаб, последние матчи, 429). До страницы — блок на /admin/config/features.\n\nКод (план). FaceitClient (HTTP + 429), FaceitIdentityService, FaceitWebhookController, FaceitRankBridge в LanMatchmakingService::rankTier. Тесты FaceitIdentityTest, FaceitWebhookTest, FaceitLfgRankTest. Не FaceitGameServerSdk.",
                        'path' => '/admin/config/features',
                        'audience' => 'Система',
                    ],
                    [
                        'title' => 'Право, ToS, 152-ФЗ',
                        'description' => "OAuth (openid) = согласие связать клубный аккаунт с FACEIT player_id и SteamID64. Elo и баны после привязки читаются публичным Data API ключом клуба, не user-token. Срок кэша профиля = пока привязка жива + 30 дней после unlink для спора матча хаба, затем чистить identities. refresh_token гостя не хранить.\n\nToS FACEIT: один человек — один аккаунт, шаринг запрещён. Клуб не выдаёт «клубный FACEIT» на смену и не хранит пароли FACEIT. Нарушение — риск бана гостя, не «фича зала». Общий NAT зала без Cyber Cafe whitelist — риск бана всей подсети, глава «Критические риски».\n\nПризы хаба/чемпионата на платформе FACEIT — их оферта. Денежные призы клуба по-прежнему только Event Manager / арена-перк (подарок заведения). Не смешивать кассу с FACEIT Points.\n\nЧужой AC не патчить, не вырезать из образа «для скорости бездиска», не отключать Secure Boot «чтобы PXE проще».\nБоевые читы на стенде клуба не держать (то же правило, что REACTOR AC).\n\nДокумент устройства: короткий абзац в оферте гостя — «привязка FACEIT добровольная, матчи FACEIT идут по их правилам и AC».",
                        'path' => '/admin/config/documents',
                        'audience' => 'Owner / Юрист',
                    ],
                    [
                        'title' => 'Фазы, оценка, приёмка',
                        'description' => "Фаза 0 (1–2 нед., техник + владелец). Не код PHP — сеть, UEFI, образ. Без закрытия рисков глава «Критические риски» фазу 0 не считать принятой.
• Тикет FACEIT Support: LAN Center / Cyber Cafe IP Whitelist на статический egress VLAN 20. Пока нет ответа — faceit off как «рейтинг».
• RB5009: Source NAT VLAN 20 на выделенный белый IP; VLAN 40 и бар не в этом NAT.
• UEFI всех игровых плат: TPM 2.0, Secure Boot; проверка PXE/CCBoot на одном ПК.
• Образ: клиент + FACEIT.sys Boot-start. Цикл: установка/Update → чистый Reboot → sc query FACEIT RUNNING → Shutdown → save в панели бездиска.
• Login шелла: junction Local + Roaming на D:/ShellData/faceit/{user_id}/…; logout: taskkill /F /IM FACEIT.exe /T, затем reg delete \"HKCU\\Software\\FACEIT\" /f, wipe D:, снять junction.
• Ярлык quick-apps валидный.
Приёмка 0: reboot, AC Running; гость A залогинился; logout; гость B не в аккаунте A (ни клиент, ни HKCU); второй ПК с образа стартует AC; с игрового ПК внешний IP = SNAT VLAN 20, не IP гостевого Wi-Fi.\n\nФаза 1 identity (2–3 нед.). App Studio: server key + OAuth (scope openid, один Redirect URI). Привязка/отвязка в ЛК. Токены гостя не пишем. Кэш Elo ключом. Шелл бейдж. LFG skill_level 1–10. Не banned. Тесты FaceitIdentityTest, FaceitLfgRankTest. Приёмка: два телефона, один player_id — второй отказ; 429 не роняет poll шелла; фича off — блока нет; take Steam клуба на ярлык FACEIT не вызывается; в faceit_identities нет refresh_token.\n\nФаза 2 hub (2–4 нед. после 1). Хаб UUID, ссылка вступить, webhook ready/finished, оверлей TV, таблица faceit_matches. Приёмка: тестовый матч хаба (не прод-рейтинг, если есть песочница; иначе late-night) рисует счёт; повтор webhook не дублирует строку; clan_war live не затирается без правила приоритета.\n\nФаза 3 championship (optional). Read-only чемпионат в карточке ивента. Без write Organizer API.\n\nОценка. 0 — образ, почти без PHP. 1 — средняя (OAuth как Telegram + HTTP-клиент). 2 — средняя (webhook + overlay). 3 — малая, если UUID руками.\nНе успех. Клубный Steam в FACEIT MM. Скрейп. Обещание «кнопка Поиск» из шелла ставит в очередь FACEIT. Свой GS. Патч AC. Смешение призов Event Manager и хаба. Общий NAT зала+Wi-Fi без whitelist. Save AC без reboot Super Client. Один junction Local. refresh_token гостя в БД.\n\nПорядок. 0 образ → 1 Connect+Data API → 2 хаб → 3 чемпионат. Не Qt-античит, не CSS-плагин.\nСвязанные главы. Античит домашней игры, пати в зале, арена, турниры, игры и лицензии, тома гибрида, Super Client, RB5009, оферта, «Критические риски».",
                        'path' => '/admin/system-tests',
                        'audience' => 'Owner / Техник / Система',
                    ],
                ],
            ...
            [
                ...
                'items' => [
                    [
                        ...
                        'description' => "Зачем. Гость должен хотеть вернуться не только из-за железа. Профиль в ЛК и шелле — витрина: аватар с рамкой, клубный статус, значки за ивенты и катки. За прогресс по клубной лестнице и боевому пропуску открываются не картинки, а скидка на ночной пакет, кешбэк часами, напиток, VIP в LFG, промокод партнёра.\n\nКоммерческий смысл. Клубные (биллинг) ачивки растят LTV бара и часов. GSI даёт вау-эффект на месте (свет, плашка). Внешние API (Steam / FACEIT / OpenDota / Riot / PUBG / Tracker) кормят статус в ЛК и XP пропуска, когда гость привязал аккаунт.\n\nЧто не цель. Второй Steam, свой античит, скрейп HTML трекеров без API-ключа, выплата «призов» с депозита игроков (это не арена-пари). Не подменять Lucky Seat, клубные промокоды /admin/promocodes и бонусы за отзывы. Не строить отдельный MM.\n\nСтатус. ТЗ / план. В проде: Quest Engine (часы / ночные визиты / визиты → депозит или фантики), аватар + стилизация, Elo арены, перк царя горы, Lucky Seat. Нет рамок, статусов, витрины значков, XP/сезонов, внешних коннекторов кроме плана FACEIT, нет вкладок админки кроме квестов.",
                        'path' => '/admin/achievements',
                        'audience' => 'Owner / Supervisor+ / Игрок / Shell',
                    ],
                    [
                        'title' => 'As-is: что уже крутится',
                        'description' => "Квесты. Таблицы achievements + user_achievements. Типы play_hours, night_visits, visit_count. Период once / weekly / monthly. Награда deposit_balance | bonus_balance после logout / expiry / force-release (AchievementService::evaluateForUser). Админка /admin/achievements — форма «Новая ачивка». Сид: 10 ч/неделя и 3 ночи → 100 ₽. Фича achievements.\n\nПрофиль. /account/profile: ник, email, POST /account/profile/avatar, галка стилизации (овал avatar_*.png, план ComfyUI на FACE-01). Телефон fixed. Ник не выбирается при SMS — DeepSeek.\n\nЛК. Кабинет рисует прогресс квестов (progress/target). Арена: ladder, week, boss, koth. Clan Wars Elo. Клипы. Telegram. FACEIT — план, не прод.\n\nШелл. ValveGsi 127.0.0.1:59898, cfg gamestate_integration_reactor.cfg, POST /api/shell/gsi (kill/death/round_win/match_win/bomb/freezetime/heartbeat). ReactiveLighting + DMX. Instant Replay по GSI-киллу. Lucky Seat за стрик. Арена судит по GSI. Трон ПК по фрагам сессии.\n\nНе путать. Бонусы за отзывы /admin/bonuses — Яндекс/2ГИС. Реестр /admin/bonus-logs — ручная выдача минут оператором (GiftTime, 1–20 мин). Промокоды RCT-* — кабинет. RX-***** — Lucky Seat на периферию магазина.",
                        'path' => '/admin/achievements',
                        'audience' => 'Supervisor+ / Система',
                    ],
                    [
                        'title' => 'Профиль: аватар, рамка, статус, витрина значков',
                        'description' => "Карточка гостя одна и та же в ЛК, шелле (login/balance/lan-live) и на idle King/оверлее TV, где уже есть аватар.\n\nСлои аватара. 1) Фото/шаблон — как сейчас (users.avatar). 2) Рамка (cosmetic frame_id) — PNG/WebP кольцо поверх, выдаётся уровнем пропуска, ачивкой или статусом. 3) Бейдж статуса под ником (короткая строка, не users.name). 4) Витрина: до 6 слотов значков, гость сам пинит в /account/profile (showcase_badge_ids). Остальные — лента «все значки».\n\nКлубные статусы (title). Не ранг FACEIT и не Elo арены. Примеры: Новичок (дефолт), Regular (N часов lifetime), Ночной волк (квест night_visits), Гладиатор (10 побед арены), Чемпион месяца (топ-1 сезона лестницы), VIP (уровень пропуска или тариф). Один активный статус. Админ задаёт приоритет: чемпион > VIP > гладиатор > …; гость не выбирает статус руками, только витрину значков.\n\nLFG. Статус VIP в поиске тиммейтов — отдельный флаг lfg_priority, не путать с FACEIT skill_level и arena Elo. Пати матчит как сейчас ±rank_delta; VIP всплывает первым в списке open-запросов при прочих равных.\n\nШелл. login payload: profile {avatar_url, frame_id, status_key, status_label, showcase[]}. Idle King по-прежнему ник+аватар; рамку рисовать, если файл локально в кэше (URL с CDN/APP_URL).\nНе затирать users.name ником Steam/FACEIT/Riot.",
                        'path' => '/account/profile',
                        'audience' => 'Игрок / Shell',
                    ],
                    [
                        'title' => 'Три уровня источников ачивок',
                        'description' => "Каждая ачивка в каталоге имеет source_kind. Облако не ходит в розетки зала: живые события приходят с шелла. Фоновые API — воркеры Laravel по привязанному identity.\n\n1) Внутриигровые (GSI / API игры на ПК).\nCS2/Dota GSI уже на шелле. Детекторы ачивок — в ValveGsi (или тонкий AchievementDetector рядом), наружу — нормализованный event в POST /api/shell/gsi {event, extra}. Облако идемпотентно пишет user_achievements + XP. Вау: ReactiveLighting, плашка в шелле, опционально клип. Не чит: Valve сама отдаёт JSON через gamestate_integration_*.cfg.\n\n2) Сторонние сервисы статистики.\nSteam Web API, FACEIT Data API (раздел «FACEIT в клубе»), OpenDota/Stratz, Riot, PUBG Developer, Tracker Network. Нужна привязка в ЛК. Синк cron, не poll шелла. Значки статусные, XP пропуска, не мигание ленты (задержка часов).\n\n3) Поведенческие (клубные).\nСвои таблицы: bookings, orders бара, referrals, computers сессии, arena_duels, tournaments. Нулевая сложность интеграции, максимальный LTV. Сейчас сюда же три типа Quest Engine — их не выкидывать, расширить типами.\n\nМатрица.\n• GSI CS2/Dota — localhost POST → шелл → /api/shell/gsi. Сложность низкая. Эффект: свет, звук, плашка.\n• Steam Web API — ISteamUserStats/GetUserStatsForGame по SteamID64. Очень низкая. Бейджи в ЛК.\n• FACEIT Data API — после OAuth Connect. Средняя. Elo/skill в профиле и LFG (уже в ТЗ FACEIT).\n• OpenDota / Stratz — REST. Средняя. Dota MMR/KDA/GPM.\n• Riot / HenrikDev — Riot ID. Средняя. Valorant/LoL ранги и серии.\n• PUBG API — SteamID или ник. Средняя. Chicken Dinner, дистанция, 10+ киллов.\n• Tracker Network — API-ключ, тег игры. Средняя. Apex/R6/Warzone без официального API.\n• Биллинг/БД — PostgreSQL. Нулевая. Часы, бар, реферал, буткемп, кочевник.\n• Арена/ивенты — ArenaDuelService, Tournament. Низкая. Гладиатор, серия, чемпион месяца.",
                        'path' => '/admin/achievements',
                        'audience' => 'Система / Supervisor+',
                    ],
                    [
                        'title' => 'GSI: «здесь и сейчас»',
                        'description' => "Детекторы живут в шелле, не в PHP. Игра шлёт JSON на 127.0.0.1:59898 (токен reactor-club). Уже есть kill, round_kills, death, bomb, flash, match/round win, Dota win/death. Расширить extra и новые event, не второй HTTP-сервер.\n\nCS2 (примеры каталога, админ включает штучно).\n• ace — round_kills ≥ 5.\n• ninja_defuse — bomb defused, игрок жив, условие «мало времени / без команды» по GSI round/bomb (если поля нет — не врать, выключить шаблон).\n• knife_or_zeus — kill при active weapon knife/taser.\n• clutch_1v3_1hp — победа раунда, hp=1, врагов в раунде было ≥3 (если allplayers нет — упростить до «win + hp≤5 + round_kills≥3»).\n• hs_streak_10 — 10 headshot подряд (player.state.round_killhs / накопление сессии).\n\nDota 2.\n• first_blood, rampage, godlike — из map/player kill_list / streak, как отдаёт GSI.\n• roshan_aegis — aegis в inventory / event.\n\nПосле детекта. 1) Свет (playLight, приоритет Round/Alert, глава «DMX»). 2) Плашка в Dashboard.qml 2–4 с. 3) POST /api/shell/gsi event=achievement extra={code,game,match_round_key}. 4) Облако: INSERT ON CONFLICT DO NOTHING в gsi_award_dedup (не whereExists) — глава «GSI spam и race».\nКухня/клип. Instant Replay уже клеит killcam — для ace можно тот же F8-пайплайн, не обязательный MVP.\nФича. achievements off или battle_pass.gsi off — детекторы не шлют award (свет по желанию клуба оставить).\nНе цель. Читать память игры, allplayers чужих ПК, VAC-риск. Только токенный GSI Valve.",
                        'path' => null,
                        'audience' => 'Shell / Игрок',
                    ],
                    [
                        'title' => 'Steam, FACEIT и официальные API игр',
                        'description' => "Привязки в /account/profile блоком «Игровые аккаунты», рядом с планом FACEIT и Telegram. Один identity на провайдера на user_id, unique external_id. Чужой клубный аккаунт с тем же id — отказ.\n\nSteam. OpenID / Steam Web API Key клуба (STEAM_WEB_API_KEY в .env, не в шелл). SteamID64 можно взять из FACEIT userinfo (steam_id_64), не заставляя второй OAuth, если FACEIT уже linked. GetPlayerAchievements / GetUserStatsForGame (CS2 app 730, Dota 570). Ачивки «стандартные Steam» — бейдж в ЛК + XP, без света. Cron reactor:sync-steam-achievements: сидящие в зале чаще, остальные раз в сутки. Не путать с пулом /admin/licenses: клубный Steam take() не является identity гостя.\n\nFACEIT. Не дублировать Connect: skill_level, elo, bans — раздел «FACEIT в клубе». Каталог: «FACEIT 10», «Elo 2000+». Синк тот же reactor:sync-faceit.\n\nDota 2 глобалка. Прямой Valve API сырых матчей закрыт. OpenDota (бесплатный REST) или Stratz. Identity: SteamID64. Ачивки: MMR 5/6/7k, KDA>10 за 5 матчей, GPM>800, «купил Рапиру и победил». Кэш opendota_snapshots. Ключ STRATZ_TOKEN опционально. Не скрейп dotabuff.\n\nRiot (Valorant / LoL). Riot ID в ЛК. Официальный Riot API (ключ клуба) или HenrikDev как адаптер, если своего ключа нет. Ачивки: Immortal/Radiant, 5 побед подряд ranked, Penta Kill LoL. Таблица riot_identities (game, puuid, riot_id). Регион выбирает гость.\n\nPUBG. PUBG Developer API, ключ PUBG_API_KEY. Ник или SteamID. Ачивки: Top 1 (пометить in_club, если матч во время активной сессии шелла — по started_at), килл 500+ м, 10+ киллов соло.\n\nНе общий cron «все 500 гостей подряд». On-demand очередь + RateLimiter — глава «Rate limit внешних API». 429 → backoff, шелл читает только кэш identities. Poll шелла внешние HTTP не делает.",
                        'path' => '/account/profile',
                        'audience' => 'Игрок / Система',
                    ],
                    [
                        'title' => 'Tracker Network и игры без API',
                        'description' => "Apex, Rainbow Six Siege, Warzone — нет простого официального API. Гость указывает тег (Apex: Nickname#1234). Синк Tracker Network API (TRACKER_API_KEY) — та же on-demand очередь, что OpenDota/Riot: активная сессия раз в 30 мин, офлайн 24–48 ч, не «все теги в одном кроне». Ачивки: топ-100 города по урону (город клуба в настройках источника), снайпер сезона, 100 побед сезона Apex.\n\nПравила. Только официальный REST, не HTML-парс tracker.gg (хрупко, ToS). Нет ключа — вкладка Источники показывает «не настроено», шаблоны source=tracker выкл. Identity unique (platform, game, tag). Гость подтверждает владение скрином/кодом в био — MVP: честность + ручной unlink админом при споре; фаза 2: OAuth платформы если появится.\nНе обещать live-плашку в шелле: задержка суточная.",
                        'path' => '/account/profile',
                        'audience' => 'Игрок / Система',
                    ],
                    [
                        'title' => 'Клубный биллинг и «железные» ачивки',
                        'description' => "Расширение Quest Engine, те же user_achievements, evaluate после сессии / заказа / реферала. Максимальный рост выручки — эти типы включать в сид сезона.\n\nGrind.\n• night_wolf — 3 ночных пакета за календарный месяц (опираться на night_visits / тариф night, окно night_start–night_end как сейчас).\n• centurion — 100 часов lifetime (play_hours once, target 100).\n• marathoner — непрерывная сессия ≥8 ч. Шелл на 7 ч 30 мин предупреждает «перерыв на разминку» (SessionAlert); ачивка всё равно засчитывается, это не наказание. Не путать с GiftTime за сбой.\n\nБар и кухня (orders + inventory, channel shell/app/site).\n• caffeine — 10 SKU категории energy/coffee за месяц.\n• fed_gamer — 5 комбо/сэндвичей (категория combo|sandwich, админ мапит SKU).\nНаграда XP + часто напиток; не двойная выдача с Lucky Seat за тот же заказ.\n\nСоциальные.\n• referral_soul — привёл друга (план: users.referred_by + первая оплаченная бронь реферала ≥60 мин). Пока реферальной ссылки нет — тип в каталоге, выдача off до фазы рефералки.\n• full_stack — бронь буткемпа на 5 мест одним BookingGroup (zone bootcamp, seats=5).\n• nomad — 10 разных computer_id в lifetime сессиях.\n\nИвенты Event Manager. Значок «участник» при roster accept, «призёр» при prizes_paid. Не путать с FACEIT championship (read-only, другой раздел).",
                        'path' => '/admin/achievements',
                        'audience' => 'Supervisor+ / Игрок',
                    ],
                    [
                        'title' => 'Арена и дуэльные ачивки',
                        'description' => "Данные уже есть: ArenaDuelService, ladder Elo, koth evening perk, week board. Не плодить второй рейтинг.\n\nКаталог.\n• gladiator — 10 побед 1v1 (kind=duel) lifetime.\n• undefeated — серия 5 побед подряд на дедике клуба без поражений (сброс на loss / tech-loss дисконнекта).\n• monthly_champion — топ-1 клубного ladder на last day сезона (job reactor:close-battle-season).\n\nПерк царя горы за вечер (час или энергетик) остаётся подарком заведения, не уровень пропуска. Ачивка «царь горы» — отдельный значок, XP. Чемпион месяца — статус + рамка, опционально бесплатный ночной пакет (настройка сезона).\nClan Wars: личный вклад уже в ЛК; значок «ветеран войны» за N live-войн — шаблон source=clan_war.\nKing of the Hill (трон ПК) — значок дневной, не статус клуба.",
                        'path' => '/admin/config/features',
                        'audience' => 'Shell / Игрок / Supervisor+',
                    ],
                    [
                        'title' => 'Боевой пропуск: XP, уровни, награды',
                        'description' => "Не картинки. Каждая завершённая ачивка даёт xp (поле каталога, 0–500). Сезон battle_pass_seasons: starts_on, ends_on, title, is_active (один live на клуб). Уровни battle_pass_levels: level, xp_required (кумулятив), reward_kind, reward_payload.\n\nТипы наград уровня (reward_kind).\n• cosmetic_frame — рамка аватара.\n• cosmetic_status — статус (если приоритет позволяет).\n• tariff_discount — скидка % на следующий ночной пакет (1 использование, TTL дней, зона/тариф в payload).\n• bar_item — бесплатный напиток: кухонный слип + списание FIFO как угощение (причина battle_pass), SKU из /admin/inventory.\n• session_minutes — кешбэк часами: те же минуты, что GiftTime, но source=battle_pass, без лимита 20 мин оператора; журнал bonus-logs.\n• lfg_vip — флаг в LFG до конца сезона или TTL.\n• partner_promo — показать код партнёра (строка, TTL, URL). Клуб не фискалит чужой товар.\n• bonus_balance / deposit_balance — как квесты сейчас. Депозит ₽ — 54-ФЗ (аванс/зачёт); фантики — не чек.\n\nПрогресс. users.battle_pass_xp, current_level. Повтор weekly-квеста каждую неделю снова даёт XP. Once — один раз. GSI ace может быть weekly, чтобы не фармили вечно без потолка: админ ставит period.\nСезон: ends_on стопает XP (season_status), не сжигает купоны в полночь. claim_grace_period 7–14 дней забрать уровни. tariff_discount / partner_promo — свой expires_at от взятия уровня (обычно 30 дней), не от ends_on. Глава «Закрытие сезона». Косметика и значки lifetime остаются. Новый сезон XP сбрасывает.\nНе путать с Lucky Seat (кейс 3 ч) и RCT-промокодами. Уровень пропуска не открывает кейс.",
                        'path' => '/admin/achievements',
                        'audience' => 'Supervisor+ / Игрок / Юридика',
                    ],
                    [
                        'title' => 'Лестница: скидки, кешбэк часами, коды партнёров',
                        'description' => "Отдельная линейка от сезона XP: пороги по арене Elo (или опционально FACEIT skill, выкл по умолчанию — не мешать FACEIT с тарифом зала). Таблица ladder_reward_tiers: metric=arena_elo|bp_level|hours_month, threshold, reward_kind, payload, is_active.\n\nПримеры сида (клуб правит на вкладке «Лестница»).\n• Elo ≥ 1100 — скидка 5% на следующую бронь ≥60 мин, 1 раз / 14 дней.\n• Elo ≥ 1300 — кешбэк 60 мин, 1 раз / 30 дней.\n• Elo ≥ 1500 или monthly_champion — ночной пакет −20% + рамка «Хищник».\n• bp_level ≥ 5 — партнёрский код (кальянная / магазин периферии / доставка), 1 на сезон.\n\nВыдача. Job при изменении Elo / закрытии сезона / evaluate. Повторно не выдавать, пока не истечёт TTL (user_reward_grants unique user+tier+period_key).\nКешбэк часами увеличивает доступное время сессии или кладёт ваучер «минуты» в кошелёк бонусного времени (тот же контур, что операторские минуты, отдельный source). Не смешивать с депозитом ₽ без чека.\nСкидка — промо с lock на user_id, type=percent, max_uses=1, не общий RCT на весь зал.",
                        'path' => '/admin/achievements',
                        'audience' => 'Supervisor+ / Игрок',
                    ],
                    [
                        'title' => 'Админка: страница настроек (какие бонусы за что)',
                        'description' => "Канон UI. Не плодить пункт сайдбара. Киберспорт → «Достижения и трофеи» /admin/achievements становится хабом на вкладках (как Avito). Supervisor+. Фича achievements скрывает весь пункт; вкладки внешних API работают, только если ключ есть (иначе плашка «не настроено»).\n\nВкладка 1. Квесты.\nТекущая форма без регресса: title, description, type play_hours|night_visits|visit_count, target, period, night window, reward deposit/фантики, sort, вкл. Счётчик выдач. Добавить поля xp (int, 0–500) и badge_id (nullable) — чтобы старые квесты кормили пропуск.\n\nВкладка 2. Каталог ачивок.\nТаблица шаблонов: code (slug unique), title, description, source_kind (gsi_cs2|gsi_dota|steam|faceit|opendota|riot|pubg|tracker|club|arena|tournament|clan_war), event/metric, target_value, period, xp, badge, reward_kind + value (может быть none — только XP/значок), is_active, sort. Фильтр по источнику. «Сид сезона» — кнопка накатывает рекомендуемый набор (Ace, Rampage, Ночной волк, Сотник, Гладиатор…) без дублей code. Редактирование порога и награды на месте: клуб сам решает, что даёт «Кофеин» — XP 20 или фри-энергетик.\n\nВкладка 3. Боевой пропуск.\nКарточка сезона: название, даты, вкл. Только один active. Градусник: сколько гостей на каком уровне (агрегат). Редактор уровней: +строка, level, XP порог, reward_kind, payload. Payload-виджеты:\n• tariff_discount — % (5–50), TTL дней, тариф/зона (night|any), max_uses=1.\n• bar_item — select SKU склада бара, количество 1.\n• session_minutes — 15/30/60/120.\n• partner_promo — код, подпись бренда, URL, TTL.\n• frame / status — select из вкладки Косметика.\n• lfg_vip — TTL дней.\nПревью «что получит гость на 5 уровне». Поля сезона: claim_grace_days (7–14, дефолт 14). Кнопка «Закрыть сезон» (confirm) — job чемпиона, XP stop, grace не режет купоны с expires_at.\n\nВкладка 4. Косметика.\nРамки: slug, имя, PNG (upload), min_level или unlock_achievement_code. Статусы: key, label, color token, priority. Значки: slug, имя, SVG/PNG, source_kind. Витрина: число слотов (3–6).\n\nВкладка 5. Лестница наград.\nСтроки tier: метрика (arena_elo / bp_level / hours_month), порог, награда (те же kind, что у пропуска), период выдачи (once / monthly), вкл. Подсказка текущих топ-10 Elo (read-only из арены).\n\nВкладка 6. Источники.\nСтатус ключей (есть/нет, без показа секрета): STEAM_WEB_API_KEY, FACEIT_API_KEY, OPENDOTA/STRATZ, RIOT, PUBG, TRACKER. Last synced_at по провайдеру. Тумблеры: gsi_awards, steam, opendota, riot, pubg, tracker. Город клуба для «топ города» Tracker. Маппинг SKU бара → caffeine/fed_gamer (multi-select категорий). Кнопка «Синхронизировать сейчас» (как отзывы) — dispatch job, не HTTP-timeout.\n\nПрава. Чтение supervisor+; смена ключей только owner (ключи живут в .env / club encrypted settings, не в git). Админ зала вкладки не видит — нет пункта.\nПути. GET /admin/achievements?tab=quests|catalog|pass|cosmetics|ladder|sources. Сохранение POST/PUT по ресурсам: /admin/achievements (квесты как сейчас), /admin/achievement-templates, /admin/battle-pass/season, /admin/battle-pass/levels, /admin/cosmetics, /admin/ladder-rewards, /admin/achievement-sources.\nНе на этой странице. RCT-промокоды, отзывы, Lucky Seat веса, фичи шелла (GSI свет) — свои экраны. Сюда только «какая ачивка / какой уровень / какой тир → какой бонус».",
                        'path' => '/admin/achievements',
                        'audience' => 'Supervisor+ / Owner',
                    ],
                    [
                        'title' => 'Модель данных и API',
                        'description' => "Расширить achievements: source_kind default club, code unique nullable (для шаблонов), xp unsigned default 0, badge_id nullable, reward_kind расширить enum (none|deposit_balance|bonus_balance|session_minutes|bar_item|tariff_discount|partner_promo|cosmetic_frame|cosmetic_status|lfg_vip), reward_payload json (в т.ч. expires_at). Старые строки: source_kind=club, xp=0 пока админ не проставит, reward_type мапится в reward_kind.\n\nНовые таблицы (черновик имён).\n• achievement_badges, cosmetic_frames, club_statuses.\n• user_identities (provider, external_id, meta json, synced_at) — steam/opendota/riot/pubg/tracker; FACEIT остаётся faceit_identities.\n• battle_pass_seasons (claim_grace_days default 14), battle_pass_levels, user_battle_pass (season_id, xp, level, claimed_level).\n• user_cosmetics (user_id, kind, ref_id, equipped_at).\n• user_badge_showcase (user_id, badge_id, slot).\n• ladder_reward_tiers, user_reward_grants (tier_id, period_key, granted_at, consumed_at, expires_at).\n• gsi_award_dedup: UNIQUE(user_id, achievement_code, match_round_key). INSERT ON CONFLICT DO NOTHING / insertOrIgnore. Чистка старше 48 ч. Не whereExists.\n\nШелл. POST /api/shell/gsi extra.{achievement_code, match_round_key}. Локальный дебаунс до freezetime. Poll balance: battle_pass {level, xp, xp_to_next, season, grace_until}, profile cosmetics, toast queue (granted[]).\nЛК. GET кабинет: то же + витрина + «забрать награду» в grace. PATCH /account/profile/showcase {badge_ids}. PATCH /account/profile/frame.\nJobs. SyncIdentityJob: очередь identities-high при login сессии (не чаще 30 мин на user+provider), identities-low офлайн 24–48 ч. RateLimiter::for opendota 50/мин, riot/pubg/tracker свои лимиты. Не reactor:sync-opendota на всю базу одним кроном. reactor:close-battle-season — стоп XP, не сжигать expires_at купонов.\nКод (план). AchievementService, GsiAchievementService, BattlePassService, IdentitySync*. Тесты: AchievementSeederTest; BattlePassGrantTest; BattlePassGraceTest; GsiAceAwardTest — три POST ace за 500 мс = один XP, unique violation не 500.",
                        'path' => '/admin/achievements',
                        'audience' => 'Система',
                    ],
                    [
                        'title' => 'ЛК, шелл, ТВ: где это видно',
                        'description' => "ЛК /account/dashboard. Карточка профиля: аватар+рамка, статус, 6 значков, градусник пропуска, кнопка «награды сезона». Блок квестов как сейчас + значок «XP +N». Лестница арены: под Elo подпись «до скидки 5% осталось X».\n/account/profile. Витрина, выбор рамки из полученных, привязки аккаунтов, «Привязать Steam / Riot / Tracker».\nШелл. Плашка GSI-ачивки. В сайдбаре мини-пропуск (уровень). Рамка на аватарке шапки. Статус под ником в пати/арене.\nTV overlay. Не спамить live-ачивки всех ПК. Только чемпион месяца / босс клуба / рамка на King of the Hill, если уже рисуем трон.\nПуш. Telegram клипов не дублировать. Опционально одно сообщение «Ace на ПК-08» в канал — тумблер на вкладке Источники, default off.",
                        'path' => '/account/dashboard',
                        'audience' => 'Игрок / Shell / Маркетинг',
                    ],
                    [
                        'title' => 'Слепая зона: GSI spam и race',
                        'description' => "Проблема. CS2 при Ace шлёт round_kills почти каждую секунду. Три POST /api/shell/gsi за 500 мс с event=achievement, code=ace на разных php-fpm: whereExists ещё не видит чужой INSERT — XP и значок трижды. То же на knife_or_zeus, если килл и round_kills приходят соседними пакетами.\n\nШелл (первый контур). После срабатывания детектора код блокируется локально до смены фазы раунда: round.phase == freezetime (или map.round увеличился). Повторный heartbeat с тем же round_kills=5 не шлёт award. Dota: блок до нового game_state / новый match_id. Дебаунс не замена БД: шелл может рестартнуться посреди раунда.\n\nБД (второй контур, канон). gsi_award_dedup UNIQUE(user_id, achievement_code, match_round_key). match_round_key = game + match_id + round (CS2) или match_id + streak_id (Dota). Вставка: INSERT ... ON CONFLICT DO NOTHING (Eloquent insertOrIgnore). XP/user_achievements писать только если insert вернул новую строку. Транзакция одна. Не SELECT FOR UPDATE без unique: два воркера оба пройдут SELECT пусто.\n\nПриёмка. Тест GsiAceAwardTest: три параллельных POST с одним match_round_key → одна строка dedup, XP один раз, HTTP 200 (не 500 на unique). Рестарт шелла в том же раунде не даёт второй ace. unique violation не логировать как error клуба.",
                        'path' => null,
                        'audience' => 'Shell / Система',
                    ],
                    [
                        'title' => 'Слепая зона: закрытие сезона',
                        'description' => "Проблема. ends_on наступил в полночь. У гостя невостребованный 5-й уровень и живая скидка на ночной пакет. Сжигать всё вместе с сезоном — претензия и потеря доверия. Копить XP после ends_on — дыра в экономику следующего сезона.\n\nДва TTL, не один.\n• season_status = live | xp_closed | archived. В секунду ends_on (или кнопка «Закрыть сезон»): live → xp_closed. evaluate / GSI / внешний синк больше не добавляют XP в этот season_id. Косметика и уже claimed уровни не трогать.\n• claim_grace_period: 7–14 дней (поле сезона claim_grace_days, дефолт 14). В ЛК кнопка «Забрать» для уровней, чей xp_required уже набран, но награда не claimed. После grace → archived, unclaimed уровни сгорают без выдачи. Минуты/напиток, которые уже в bonus-logs / складе, не отбирать.\n\nКупоны отдельно от сезона. tariff_discount и partner_promo при claim пишут user_reward_grants.expires_at = now + payload.ttl_days (сид 30 дней), не ends_on сезона. Скидка, взятая 29-го числа сезона, жива ещё 30 дней. Проверка на бронь: expires_at > now AND consumed_at is null. lfg_vip: либо payload.ttl_days, либо до xp_closed+grace — что короче; в payload явно.\n\nНовый сезон. Новый battle_pass_seasons.is_active. XP с нуля. Не копировать unclaimed уровни. Рамки/значки lifetime остаются в user_cosmetics.\nПриёмка. BattlePassGraceTest: xp_closed, гость claimит скидку на день 3 grace — ок; день 15 — отказ. Купон expires_at через 30 дней после claim, сезон уже archived — скидка на бронь ещё действует.",
                        'path' => '/admin/achievements',
                        'audience' => 'Supervisor+ / Игрок / Юридика',
                    ],
                    [
                        'title' => 'Слепая зона: rate limit внешних API',
                        'description' => "Проблема. OpenDota / Riot / PUBG / Tracker режут RPS. Крон reactor:sync-opendota на 500 identities за минуту = пачка 429 и бан ключа по IP клуба (облако, не зал). Poll шелла от этого не должен страдать, но очередь забьётся, ачивки MMR «залипнут».\n\nКанон — не полный обход базы.\n• Login / heartbeat сессии: SyncIdentityJob в identities-high, не чаще 30 мин на пару user_id+provider (кэш last_synced_at).\n• Офлайн: identities-low, 24–48 ч, джиттер, батч с паузой.\n• «Синхронизировать сейчас» в админке — тот же job, не HTTP-timeout на 500 человек: dispatch, тост «в очереди».\n• RateLimiter::for('opendota') 50/мин на воркер (или лимит ключа, что строже). riot / pubg / tracker — свои ключи. 429 → release(backoff 60–300 с), не failed сразу. После 5×429 — инцидент quality, не молча.\n• Шелл и /api/shell/balance читают только кэш identities. Внешний HTTP из php-fpm запроса гостя запрещён.\nНе цель. Скрейп HTML как обход 429. Второй ключ «на всякий» в git.",
                        'path' => '/admin/achievements',
                        'audience' => 'Система / Owner',
                    ],
                    [
                        'title' => 'Слепая зона: DMX и одновременные события',
                        'description' => "Проблема. knife_or_zeus (2 с строб) и одновременно уровень BP / night_wolf (4 с заливка). Два playLight подряд: контроллер Art-Net остаётся в цикле первого или смешивает цвета. То же Ace + killcam-свет + ambient зала.\n\nКанон — уже есть enum ReactiveLighting::Priority, не плодить второй стек. Маппинг ачивок:\n• GSI live (ace, knife, rampage, first_blood) → Round (70) или Alert (90), 2–3 с.\n• Клубная ачивка / уровень BP / плашка пропуска → Event (50), ~4 с.\n• Фоновый зал / карта → Ambient (20).\n• Idle (0) — ничего не играет.\nНовый пресет прерывает текущий только если pri строго выше. Равный и ниже — в очередь или drop (drop безопаснее, чем зависший цикл: очередь не копить больше 1). playPreset уже принимает Priority — детектор ачивок не звать pulse в обход.\nЗвук/плашка QML тем же правилом: GSI тост бьёт BP-тост, не наоборот.\nНе слать DMX с облака: шелл единственный painter (как сейчас в шапке ReactiveLighting). Облако в gsi-ответе может дать light_hint, шелл сам решает pri.\nПриёмка. Эталон: ace + одновременно evaluate night_wolf → сначала Round 2–3 с, затем Event, ambient не перебивает Round. Два ace в одном раунде — один свет (тот же дебаунс, что award).",
                        'path' => null,
                        'audience' => 'Shell / Техник',
                    ],
                    [
                        'title' => 'Сводка наград и проводок',
                        'description' => "Каждый reward_kind — отдельная проводка. Не класть минуты на депозит ₽ и не фискалить фантики. Подробности чека — глава «54-ФЗ».\n\n• cosmetic_frame / cosmetic_status — нефискально. Контент клуба (PNG/SVG). Хранение user_cosmetics. Не сгорает с сезоном.\n• session_minutes — подарок заведения, не выручка. Бонусный фонд минут. Журнал bonus-logs source=battle_pass. Не лимит GiftTime 1–20 мин оператора.\n• bar_item — угощение, COGS клуба. Склад бара FIFO. stock_movements reason=battle_pass + кухонный слип. Нет SKU → фолбэк XP или фантики, не «виртуальный энергетик».\n• tariff_discount — скидка на следующий чек (база меньше), не отдельный чек на скидку. Персональный одноразовый промо user_id lock, max_uses=1, expires_at свой. Не общий RCT.\n• partner_promo — бартер, клуб чужой товар не продаёт. Показ кода в ЛК, user_reward_grants, expires_at свой.\n• bonus_balance — виртуальные баллы (фантики), внутренний счёт, без чека.\n• deposit_balance — деньги кошелька, 54-ФЗ как квест сейчас (аванс/зачёт).\n• lfg_vip — нефискально, флаг до TTL/grace.\n\nПорядок grant: 1) unique/идемпотентность 2) проводка склада/минут/промо 3) XP (если ачивка) 4) тост. Откат склада при падении после insert cosmetics не оставлять «напиток списан, рамки нет»: одна DB-транзакция, слип кухни после commit.",
                        'path' => '/admin/bonus-logs',
                        'audience' => 'Owner / Юрист / Система',
                    ],
                    [
                        'title' => '54-ФЗ, риски, что не делать',
                        'description' => "Деньги. Проводки по kind — глава «Сводка наград и проводок». deposit_balance — как квест сейчас, чек по правилам кошелька. Фантики — не фискалить. Минуты battle_pass / лестницы — подарок заведения, журнал bonus-logs source=battle_pass, не «выручка минус». Напиток — угощение (stock_movements reason=battle_pass), COGS клуба. Скидка — меньше база при следующей оплате, чек на фактическую сумму. Код партнёра — клуб не продаёт чужой товар, только показывает строку.\n\nАнтифарм. GSI only при живой сессии шелла (booking active). Внешний API — identity unique, ручной unlink. Реферал — награда после первой оплаченной брони друга, не за SMS. Марафон 8 ч — предупреждение о разминке, не скрытый штраф.\n\nСекреты. API-ключи только .env / encrypted club settings. Шелл ключей не видит. Tracker без ключа выкл.\n\nНе цель.\n• Скрейп HTML FACEIT/Tracker/Steam.\n• Второй рейтинг вместо Arena Elo / FACEIT.\n• Призовой фонд с депозитов гостей за «ачивку».\n• Обещание Immortal в Valorant без Riot identity.\n• Путать пропуск с Lucky Seat и RCT.\n• Патчить игры ради событий, которых нет в GSI.\n• Отдавать XP за клубный Steam из /admin/licenses (это не гость).\n• Хранить пароли Steam/Riot.\n\n152-ФЗ. Привязка = согласие связать клубный аккаунт с внешним id. Срок кэша как FACEIT: пока linked + 30 дней после unlink, затем чистить snapshots. Оферта: короткий абзац «ачивки и пропуск — программа лояльности клуба, внешние ранги по правилам платформ».",
                        'path' => '/legal/offer',
                        'audience' => 'Owner / Юрист',
                    ],
                    [
                        'title' => 'Фазы, оценка, приёмка',
                        'description' => "Фаза 0 (3–5 дн.). Схема БД + вкладки админки (квесты остаются рабочими). Поля xp/badge на существующих квестах. Пустые каталог/сезон/лестница без внешних API. Тесты старых AchievementSeederTest / evaluate зелёные. Приёмка: создать квест как сейчас; открыть tab=pass — пустой сезон, не 500.\n\nФаза 1 косметика + пропуск из клубных квестов (1–2 нед.). Рамки/статусы/витрина. Сезон с 5–8 уровнями. XP с play_hours/night_visits. Награды: frame, session_minutes, bar_item, tariff_discount. ЛК градусник, шелл payload profile. Приёмка: 10 часов за неделю → XP → уровень → минуты в bonus-logs source=battle_pass; рамка в шелле; скидка один раз на ночной тариф.\n\nФаза 2 арена/ивенты/бар (1 нед.). Шаблоны gladiator, caffeine, nomad, tournament participant. Сид кнопкой. Приёмка: 10 дуэлей → значок; 10 энергетиков → XP; буткемп 5 — full_stack.\n\nФаза 3 GSI live (1–2 нед.). Детекторы ace/knife/rampage/first_blood + свет Round/Alert + плашка + UNIQUE dedup. Приёмка: три POST ace за 500 мс = 1 XP (GsiAceAwardTest); катка на эталоне — один свет за раунд, не 30 heartbeat; night_wolf+ace — Round затем Event (глава «DMX»).\n\nФаза 4 Steam + FACEIT бейджи (после FACEIT identity). GetUserStats + каталог FACEIT 10. Приёмка: без take() клубного Steam.\n\nФаза 5 OpenDota / Riot / PUBG (по ключам, 1–2 нед. каждый). Только identities-high/low + RateLimiter, не полный крон. Приёмка: 50 запросов/мин OpenDota, 429 не валит poll шелла. Фаза 6 Tracker. Фаза 7 рефералка и partner_promo витрина. Grace сезона — BattlePassGraceTest до фазы 1 включить в приёмку вкладки Pass.\n\nНе успех. Сломанные старые квесты. Двойная выдача с Lucky Seat. Секрет ключа в JSON шелла. Скрейп. XP за чужой Steam пула. Админ зала правит сезон. Пустой club_features выключает ачивки (как сейчас: пустой = всё вкл).\n\nПорядок. 0 схема/админка → 1 XP из биллинга → 2 арена/бар → 3 GSI вау → 4 Steam/FACEIT → 5–6 внешние → 7 партнёры/реферал.\nСвязанные главы. Достижения (киберспорт), профиль, FACEIT, арена, Lucky Seat, промокоды, бонусы за отзывы, бар/склад, 54-ФЗ кошелёк, ПК лиц (аватар), фичи.",
                        'path' => '/admin/system-tests',
                        'audience' => 'Owner / Система',
                    ],
                ],
            ...
            [
                ...
                'items' => [
                    ...
                    [
                        'title' => 'Игры на ПК',
                        'description' => 'Список игр, топы, запись запуска, take/free аккаунта, pause/unpause (новый PIN), обновление VDF-кэша. GET /api/shell/games передаёт terminal_id: если ПК в активном турнире с lock_games, каталог и featured сужаются до игры ивента. Ярлык FACEIT (quick-apps) take клубного Steam не делает — личный аккаунт гостя. План — раздел «FACEIT в клубе».',
                        'path' => null,
                        'audience' => 'Shell',
                    ],
                    [
