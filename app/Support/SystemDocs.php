        ...
        return [
            ...
            [
                ...
                'items' => [
                    [
                        'title' => 'Инциденты',
                        'description' => "Единая лента /admin/incidents: late_order, low_stock, расхождения склада, SOS, HID, ручные записи, shell-инциденты (fan_bearing_wear, golden_image_drift, nic_link_flap — «Заменить патч-корд на ПК-ХХ»).\n\nТипы с ПК: POST /api/shell/incidents (ShellIncidentService, dedupe по computer_id+type пока не закрыт). Link flap ≥2 за смену пишет nic_link_flap из heartbeat (см. «Деградация кабеля» в Shell).\n\nAck и закрытие (resolve — supervisor+).",
            ...
            [
                ...
                'items' => [
                    ...
                    [
                        ...
                        'description' => "Зачем. После обновления игр на одном ПК (обычно Super Client) остальные SSD-кэши не должны качать тот же билд с WAN — канал клуба забивается. Контур раздаёт уже лежащие на D: файлы по VLAN.\n\nЭто не SteamCDN, не Lancache и не BitTorrent: оркестрация через heartbeat booking, раздача — HTTP seed на шелле.\n\nПоток\n1) Seed: ПК с Super Client (или уже слушающий сид) поднимает LocalHttpServer на всех IPv4, порт из config.ini [PatchCache] port=8745.\n2) Heartbeat шлёт lan_ip + patch_seed_port; сервер отвечает patch_seed {enabled, port}.\n3) Peer: LanPatchService сравнивает games_inventory (platform/appId/buildId). Если у сида build новее — в ответ heartbeat кладётся patch_pull {command_id, apps:[{p,id,b,peer_ip,peer_port}]}.\n4) Шелл (PatchCacheCoordinator) GET /manifest/{p}/{id} и /file/... с сида, копирует отличающиеся файлы на локальный install root (Steam steamapps/common или Epic InstallLocation).\n5) Ack: patch_pull_ack_id + result/message в следующем heartbeat; idle-shutdown не шлётся, пока pull в очереди.\n\nГарды. Гостевая сессия на peer → busy_session, pull откладывается. На seed во время гостя сид гасится (кроме Super Client). Один живой сид в клубе предпочтительно — SC; иначе любой онлайн с port>0 и lan_ip.\n\nЛимиты config.ini [PatchCache]: enabled, port, max_files=400, max_file_mb=512 — крупные депоты целиком не гоняются, это дифф свежих файлов манифеста.\n\nАдминка: на плитке/карточке ПК — seed :port и lan-pull #id · result. Тесты: tests/Feature/ShellLanPatchAndLinkFlapTest.php.\nКод: shell PatchCacheCoordinator + LocalHttpServer::listenAny; booking LanPatchService, ComputerPowerService::heartbeat.",
                        'path' => '/admin/dashboard',
                        'audience' => 'Техник / Shell',
                    ],
                    [
