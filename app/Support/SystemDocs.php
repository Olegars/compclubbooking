            ...
            [
                ...
                'items' => [
                    ...
                    [
                        ...
                        'description' => "Стек. C# / .NET 8, CounterStrikeSharp на Metamod:Source. Модуль ReactorAC Gate. Секрет и URL API только в конфиге дедика, не в клиенте. Booking сам на UDP сервера не ходит.\n\nЛовушка sv_password. Если нативному sv_password задать общий пароль, Source 2 отсеет игрока с индивидуальным token до хука плагина. В режиме gate sv_password держать пустым (\"\"), либо перехватить OnValidateServerPassword. Проверку пароля делегирует плагин: пароль подключения (инфо-строка / INetChannel, не кастомный setinfo-JWT). Пустой или не похож на token — сразу kick, без HTTP.\n\nЖизненный цикл. Клиент: connect ip:port; password TOKEN → движок (sv_password пуст) → CSS до спавна и занятия слота. Целевой хук: OnClientAuthorized (slot, steamId) или более ранний PreConnect, если API версии CSS его даёт стабильно. EventPlayerConnectFull — поздно: слот уже занят. Ботов не валидировать.\nПоток хука:\n1) Есть запись ConnectedPlayers для этого SteamID64 после успешного validate этой карты/матча → не требовать новый token (changelevel). Иначе дальше.\n2) Src IP из подсети зала — не return: POST /api/ac/server/validate-station {client_ip, steam_id}. 200 только если в шелле активная сессия и гость привязал этот SteamID. Иначе kick даже из VLAN 20. Нет привязки SteamID в ЛК/шелле — kick «привяжите Steam».\n3) Домашний: прочитать token подключения; пустой / не формат — kick без HTTP.\n4) Сразу is_validating=true; jointeam/joinclass глушить CommandListener, пока нет allow. HTTP POST /api/ac/server/validate в Task.Run (не блокировать тик).\n5) Кик с главного потока: Server.NextFrame. 200 → ConnectedPlayers[steam_id]={session_id, validated_at}, token consumed. 403 / таймаут → kick fail-closed.\n\nKeep-alive. Таймер CSS ~30 с: все не-боты SteamID64 → POST /api/ac/server/heartbeat-check. Дом: нет пульса службы AC > ~25 с → kick. Зал: нет active-сессии шелла / сменился Steam → kick. Changelevel не чистит ConnectedPlayers.\n\nДва TTL в облаке. token_connect_ttl (60–120 с) — выдать connect и пройти handshake. session_ttl — пока жив heartbeat службы. Handshake на 115-й секунде connect_ttl: validate ok, статус consumed сразу; дальше держит session_ttl, не connect_ttl. Иначе медленный HDD/загрузка карты сжигает токен до validate.\n\nПерезаход. Токен одноразовый: старая строка из консоли после краша CS2 не работает. Трей клиента: «Переподключиться к матчу» → новый connect_token на тот же match_id (матч жив, не бан, heartbeat alive) и копирует connect в буфер.\n\nКод (целевой): ReactorAcGatePlugin; ConnectedPlayers; CommandListener jointeam; ValidateTokenAsync; ValidateStationAsync. Тесты: AcConnectTokenTest, AcValidateStationTest, AcHeartbeatCheckTest. Стенд дедика: чужой token, VLAN без сессии шелла, VPN-префикс, changelevel, kill службы, jointeam за 200 мс.",
                        'path' => '/admin/tournaments',
                        'audience' => 'Техник / Система',
                    ],
                    [
                        'title' => 'Слепые зоны gate: зал, jointeam, MITM, changelevel',
                        'description' => "1) station_trusted не по IP. Дедик dual-homed (WAN для дома + VLAN 20 для зала): player.IpAddress может быть не 192.168.20.*. Домашний VPN/туннель может сымитировать клубный префикс. StartsWith(192.168.20.) недостаточно и опасно как единственный bypass AC.\nКанон. Подсеть зала — только эвристика «кандидат в зал». Плагин всегда POST /api/ac/server/validate-station {client_ip, steam_id, server_secret}. Облако: с этого IP (или с computer_id, который heartbeat шелла только что прислал с этим lan_ip) прямо сейчас active-сессия, и users.steam_id / привязка сессии = этот SteamID64. Нет сессии или чужой Steam — kick даже из VLAN. Привязка Steam: ЛК или шелл до выхода на дедик (поле в профиле / login-пакет); без неё зал на дедик не пускать.\n\n2) Окно OnClientAuthorized → HTTP (100–400 мс, при лаге API дольше). Движок уже считает игрока подключенным: успевает jointeam/спавн.\nКанон. В хуке сразу is_validating=true. CommandListener: jointeam, joinclass, и аналоги выбора стороны — return HookResult.Handled, пока нет allow. Спавн в команде до allow — kick или вернуть в spectate. После allow флаг снять. Таймаут validate fail-closed.\n\n3) Перехват token. Локальный Fiddler/mitmproxy + свой Root CA в Windows: клиент AC отдаёт connect_token другу или второй машине с читом.\nКанон. SSL Pinning в REACTOR AC на /api/ac/*: сверка SPKI/открытого ключа сертификата APP_URL, игнор User Root CA. Сломан пиннинг (подмена сертификата) — нет token, UI «нельзя проверить сервер». Плагин по-прежнему биндит token к steam_id: украденный token с другого Steam не проходит validate. Пиннинг закрывает «снял token с провода», биндинг — «отдал другу».\n\n4) Changelevel. Движок переподключает всех. Token уже consumed. Повторный validate пароля выкинет весь лобби.\nКанон. После первого allow: ConnectedPlayers[steam_id] = {session_id, validated_at, kind=home|station}. OnClientAuthorized при штатной смене карты: есть запись и облачный heartbeat-check ещё ok → не требовать новый connect_token. Hot-reload плагина стирает память: тогда опереться на облачную session (не на token). Полный дисконнект / краш клиента — не changelevel: нужен новый token (кнопка «Переподключиться»).\nHeartbeat-check гоняет и зал: logout шелла → kick со дедика, даже если IP ещё клубный.",
                        'path' => '/admin/tournaments',
                        'audience' => 'Техник / Система',
                    ],
                    [
                        'title' => 'Фазы 0–2: оферта, Protocol Gate',
                        'description' => "Фаза 0 (1–2 нед.). Оферта: клиент на домашний ПК, HWID, скрины, скоупы бана. Kick-тексты. Фича reactor_ac=off: пока off — поведение как сейчас (это долг: общий пароль ещё жив). Стек дедика зафиксировать: CounterStrikeSharp. Приёмка: заглушка /ac/download, документ в /admin/config/documents.\n\nФаза 1 (часть окна 3–4 мес. MVP). Служба + heartbeat + integrity, без kick на дедике. POST /api/ac/heartbeat. Отказ заранее логировать: testsigning, VM. ЛК: «AC онлайн / не установлен». Fair-play — лента сессий. Тесты AcClientSessionTest.\n\nФаза 2. Protocol Gate. sv_password=\"\" , OnClientAuthorized, is_validating+блок jointeam, validate-station, SSL pinning, ConnectedPlayers на changelevel, heartbeat-check 30 с.\nПриёмка 2: дома без AC не зайти; чужой SteamID на token — kick; consumed token — kick; handshake на исходе connect_ttl — ok; VLAN без сессии шелла — kick; сессия шелла + свой Steam — зайти без token; jointeam до allow не проходит; changelevel не кикает лобби; kill службы 40 с — kick; краш CS2 — «Переподключиться»; mitm Root CA — клиент не отдаёт token.",
                        'path' => '/admin/config/features',
                    ...
                    [
                        ...
                        'description' => "Фича reactor_ac: mode=off|telemetry|gate, token_connect_ttl_sec (60–120), session/heartbeat_stale_sec (~25), allow_station_without_ac (default true), club_subnet (192.168.20.0/24), evidence_keep_days (90). Пустой club_features AC не включает.\n\nAPI клиента (Bearer игрока):\n• POST /api/ac/login;\n• POST /api/ac/heartbeat;\n• POST /api/ac/connect-token {match_id} — token только если heartbeat alive и integrity ok; тот же match_id для «Переподключиться», если матч жив и не бан;\n• POST /api/ac/events, POST /api/ac/evidence;\n• GET /api/ac/policy.\nAPI дедика (server_secret):\n• POST /api/ac/server/validate {steam_id, token, ip} → allow (consumed) | deny_*;\n• POST /api/ac/server/validate-station {client_ip, steam_id} → allow только active-сессия шелла + тот же SteamID, иначе deny (VLAN без сессии / VPN-префикс);\n• POST /api/ac/server/heartbeat-check {steam_ids[]} → kick[] дом (нет пульса AC) и зал (нет сессии шелла);\n• kick-ack.\nFail-closed: таймаут = deny.\nАдминка: /admin/fair-play. Дистрибутив: /ac/download, /ac.json.\n\nТаблицы: ac_sessions, ac_hwids; ac_tickets (…, connect_expires_at, consumed_at, session до heartbeat); ac_events, ac_evidence; ac_bans scope=match_making|tournament|full_ban.\n\nКод: reactorac; ReactorAcGatePlugin (CSS); booking AcTicketService / AcServerGuard. Тесты: SystemDocs, AcConnectTokenTest, AcHeartbeatCheckTest, AcBanTest.",
                        'path' => '/admin/config/features',
                        'audience' => 'Система',
                    ],
                    [
