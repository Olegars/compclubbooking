            ...
            [
                ...
                'items' => [
                    ...
                    [
                        ...
                        'description' => "Три контура. Booking в розетки дома не ходит — клиент и плагин сами ходят HTTPS к APP_URL.\n\n1) Клиент REACTOR AC (репо не Qt-шелл). MSI/EXE, служба Windows + трей. Без своего .sys на MVP. Задачи: логин ЛК, HWID, heartbeat, integrity (testsigning / VM / публичный скан), запрос connect_token, evidence по команде облака.\n\n2) Облако. Связка на 60–120 с: match_id + steam_id + connect_token + client_heartbeat_alive. Токен — короткий alphanumeric, одноразовый. Не JWT в setinfo и не client convar: в CS2 кастомные клиентские convar жёстко режет Source 2, длинный токен дропается, правка клиента игры = риск VAC. Пароль сервера = этот токен.\n\n3) Дедик CS2. Стек зафиксирован: CounterStrikeSharp (C# на Metamod:Source) — де-факто стандарт моддинга CS2, async HTTP к REST. На авторизации игрока плагин берёт steam_id и sv-пароль, с которым тот зашёл, POST /api/ac/server/validate {steam_id, token, server_secret}. 403 → kick. Общий пароль лобби исключается полностью. Секрет сервера только на дедике и в .env.\n\nКлиент не банит сам. station_trusted — отдельная ветка validate для зала.",
                        'path' => '/admin/config/features',
                        'audience' => 'Система',
                    ],
                    [
                        'title' => 'Клиент: установщик и служба',
                        'description' => "Дистрибутив. GET /ac/download после логина ЛК, манифест GET /ac.json {version, url, sha256, min_os}. Подпись Authenticode (не EV-драйвер). Автообновление до матча, не в раунде.\n\nУстановка MVP. Служба, не kernel. Win10/11 x64. Имя службы ReactorAcSvc, старт Automatic. Трей: логин, «готов к серверу», целостность. Токен не выдаётся, если testsigning, VM/гипервизор или служба не pulsing.\n\nHWID: digest SMBIOS UUID + диск системного тома + постоянный NIC + MachineGuid, salt версии. Не сырые серийники в публичном JSON.\n\nEvidence (фаза 3): скрин, список процессов (имя + sha256), без дампа RAM, ≤3 МБ. Не в /clips и не в Telegram.\n\nРепозиторий клиента C:\\Qt\\reactorac (целевой). Плагин дедика — отдельный проект CounterStrikeSharp. Свой .sys не класть в эти репо до решения по фазе 4.",
                        'path' => '/ac/download',
                        'audience' => 'Игрок / Техник',
                    ],
                    [
                        'title' => 'Match Connect Token (не convar, не общий пароль)',
                        'description' => "Сейчас. ArenaDuelService::connectUri клеит ARENA_CS2_CONNECT + общий password. Это дыра: кто увидел URI, заходит без AC.\n\nЗапрещено в ТЗ. Передача длинного ticket через client convar / setinfo: Source 2 режет кастомные клиентские convar, токен дропается, правка клиента CS2 рискует VAC. Whitelist SteamID с облака на дедик — запасной костыль, не основной путь (лаг, не кикает мёртвый агент).\n\nКанон фазы 2. Match Connect Token + SteamID Binding.\n1) Клиент AC (heartbeat alive, integrity ok) POST /api/ac/connect-token {match_id}.\n2) Облако кладёт ac_tickets: match_id, user_id, steam_id, connect_token (короткий alphanumeric), exp 60–120 с, одноразовый.\n3) Игрок жмёт connect: штатная команда `connect ip:port; password <connect_token>`. Хост из ARENA_CS2_CONNECT, пароль — только его токен.\n4) sv_password на дедике в режиме AC не общий: CSS принимает игрока и читает пароль, с которым он авторизовался (индивидуальный), плюс SteamID.\n5) POST /api/ac/server/validate {steam_id, token, server_secret}. Нужны все: токен существует, не истёк, steam_id совпал, heartbeat клиента ещё alive. Иначе kick «установите античит» / «токен недействителен».\n6) Повторно тот же токен не принимается. Перезаход — новый токен.\n\nЗал. station_trusted: живая сессия шелла + IP VLAN 20, без домашнего токена. Смешанный матч допустим.\n\nЖелезо дедика. Не бездиск .10. UDP игрокам в WAN, исходящий HTTPS к APP_URL. CSS не светит админку наружу.\n\nDota — второй контур после CS2. MVP только CS2 + CounterStrikeSharp.",
                        'path' => '/admin/tournaments',
                        'audience' => 'Техник / Система',
                    ],
                    [
                        'title' => 'Фазы 0–2: оферта, Protocol Gate',
                        'description' => "Фаза 0 (1–2 нед.). Оферта: клиент на домашний ПК, HWID, скрины, скоупы бана. Kick-тексты. Фича reactor_ac=off: пока off — поведение как сейчас (это долг: общий пароль ещё жив). Стек дедика зафиксировать: CounterStrikeSharp. Приёмка: заглушка /ac/download, документ в /admin/config/documents.\n\nФаза 1 (часть окна 3–4 мес. MVP). Служба + heartbeat + integrity, без kick на дедике. POST /api/ac/heartbeat. Отказ заранее логировать: testsigning, VM. ЛК: «AC онлайн / не установлен». Fair-play — лента сессий. Тесты AcClientSessionTest.\n\nФаза 2. Protocol Gate. Общий пароль лобби выключается. Connect только с connect_token. CSS validate {steam_id, token}. Heartbeat мёртв или integrity fail → токен не выдаётся. Станция зала — station_trusted.\nПриёмка 2: дома без AC не зайти; дома с AC — зайти; украденный чужой token + другой SteamID — kick; повтор того же token — kick; PC-08 без домашнего AC — зайти.",
                        'path' => '/admin/config/features',
                    ...
                    [
                        ...
                        'description' => "Фича reactor_ac: mode=off|telemetry|gate, token_ttl_sec (60–120), heartbeat_stale_sec (~20), allow_station_without_ac (default true), evidence_keep_days (90). Пустой club_features AC не включает.\n\nAPI клиента (Bearer игрока):\n• POST /api/ac/login;\n• POST /api/ac/heartbeat — status, version, hwid, integrity (testsigning, vm, findings);\n• POST /api/ac/connect-token {match_id} — выдаёт короткий token только если heartbeat alive и integrity ok;\n• POST /api/ac/events, POST /api/ac/evidence;\n• GET /api/ac/policy.\nAPI дедика (server_secret): POST /api/ac/server/validate {steam_id, token} → allow | deny_no_ac | deny_banned | deny_stale | deny_wrong_match | deny_steam_mismatch; kick-ack.\nАдминка: /admin/fair-play, pardon. Дистрибутив: /ac/download, /ac.json.\n\nТаблицы (черновик):\n• ac_sessions, ac_hwids;\n• ac_tickets (match_id, user_id, steam_id, connect_token, exp, used_at);\n• ac_events, ac_evidence;\n• ac_bans: scope=match_making|tournament|full_ban, until, reason. full_ban на зал только руками supervisor.\n\nКод: reactorac (клиент), CSS-плагин на дедике, booking AcTicketService / AcServerGuard / AcVerdictService. Не Qt-шелл. Тесты: SystemDocs, позже AcConnectTokenTest, AcServerValidateTest, AcBanTest.\nНе в git: дампы, сигнатуры приватного софта, исходник чужого/своего драйвера.",
                        'path' => '/admin/config/features',
                        'audience' => 'Система',
                    ],
                    [
                        'title' => 'ЛК, админка, сервер, зал',
                        'description' => "ЛК /account/arena. Баннер скачать AC, статус готов/не запущен/бан (с скоупом). Connect показывает персональную строку с одноразовым password; общий пароль лобби не светить. Пока AC offline — кнопки нет. Бар и бронь ПК от AC не зависят, кроме full_ban.\n\nСайт: «Играй с дома», Win10/11, что это Protocol Gate на сервер клуба, не античит уровня FACEIT.\n\n/admin/fair-play: сессии, HWID, кики CSS, evidence, смена scope, pardon. Дашборд зала — счётчик домашних на сервере, не flap/drift.\nШелл /api/ac/* не вызывает. TV античит не показывает.\nДенежные призы: в ЛК залог/паспорт до выдачи connect_token на турнир (фаза 3), не фаза 4.",
                        'path' => '/account/arena',
