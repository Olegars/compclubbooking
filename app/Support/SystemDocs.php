            ...
            [
                ...
                'items' => [
                    ...
                    [
                        ...
                        'description' => "Отдельный античит на домашний ПК игрока: ставится как FACEIT AC, работает сервисом + драйвером, без него игровой сервер клуба (CS2/Dota, ARENA_CS2_CONNECT) в матч не пускает. Бан — аккаунт (телефон) и отпечаток домашнего железа. Не модуль PC Shell и не киоск зала. Полный план — раздел «Античит домашней игры (REACTOR AC)».",
                        'path' => '/admin/docs',
                        'audience' => 'Supervisor+ / Игрок / Система',
                    ],
                ],
            ...
            [
                ...
                'items' => [
                    [
                        ...
                        'description' => "Зачем. Игрок со своего домашнего Windows ставит клиент REACTOR AC (как FACEIT AC) и только с живым античитом попадает на игровой сервер клуба — CS2/Dota, адрес из ARENA_CS2_CONNECT / ARENA_DOTA_CONNECT. Без клиента строка connect с паролем арены бесполезна: плагин сервера кикает. Так клуб открывает рейтинг, турниры и миксы не только гостям зала, но и тем, кто сидит дома.\n\nЧто защищаем. Соревновательные слоты на своих дедиках: арена из ЛК, межклубный турнир, в перспективе ranked-очередь. Призы и Elo нельзя честно считать, если домашний читер заходит по IP сервера.\n\nЧто не цель. PC Shell, киоск, drift D:, камеры зала, VAC/EAC/Vanguard. Игрок в клубе за своим местом на VLAN 20 может ходить на тот же дедик как trusted station (терминал + сессия) — домашний путь другой: враждебная машина, у игрока админ.\n\nНе путать с FACEIT-платформой: мы не подменяем их античит и не пускаем на сервера FACEIT. Свой дедик, свой аккаунт (телефон 0451), свой бан.\n\nСтатус. ТЗ / план. Нет установщика, нет /api/ac/*, нет плагина CS2, нет фичи reactor_ac. Скачать нечего. Ниже — слои как у ACE, фазы, API, право, приёмка.",
                        'path' => '/account/arena',
                        'audience' => 'Owner / Игрок / Система',
                    ],
                    [
                        'title' => 'Как у FACEIT AC: те же слои',
                        'description' => "Публичная схема FACEIT ACE — целевая архитектура. Бинари и драйвер не копируем и не реверсим.\n• установщик на домашний Windows (права администратора при установке);\n• служба в user-mode, автозагрузка, жива до connect;\n• kernel-драйвер: на враждебной машине без него службу снимают;\n• облако: сессия, сердцебиение, вердикт, бан аккаунта + HWID;\n• игровой сервер не пускает без валидного тикета античита;\n• скрин и список процессов для апелляции.\n\nПоток. 1) Скачал REACTOR AC с сайта или ЛК. 2) Установил, при драйвере — reboot. 3) Вошёл тем же аккаунтом, что ЛК (телефон). 4) Клиент получил match ticket. 5) Steam/CS2 connect на дедик клуба. 6) Плагин сервера спросил облако: тикет жив, HWID не в бане, агент пульсирует. 7) Нет — kick «установите античит клуба». 8) Нарушение в матче — kick + пакет evidence + бан аккаунта и железа.\n\nЗал. ПК клуба на тот же дедик: либо станция в allowlist (активная сессия шелла = trusted), либо тот же клиент в образе. Бан HWID не вешать на PC-08 — общая машина. Trusted-station помечается computer_id, не домашним отпечатком.",
                        'path' => '/account/arena',
                        'audience' => 'Owner / Игрок',
                    ],
                    [
                        'title' => 'Модель угроз домашнего ПК',
                        'description' => "Машина игрока. Админ у него, читы ставятся заранее, DMA и прошивки мыши переживают reboot, античит можно выгрузить. Это та же модель, что у FACEIT, не клубный бездиск.\n\nКласс A — user-mode в матче. Посторонний процесс, оверлей ESP, инжект в cs2.exe, патч бинарника игры, макросы AHK, второй клиент-радар. Закрываем агентом + тикетом сервера (фазы 1–2).\n\nКласс B — обход клиента. Убить службу и зайти по IP:порту с паролем из ЛК. Пароль арены один на лобби — его недостаточно. Сервер принимает только игрока, чей SteamID/ник склеен с живым ticket. Bypass: подмена агента своим exe. Лечится подписью клиента, pinning версии, kernel (фаза 4).\n\nКласс C — железо. DMA-плата, прошивка мыши с ассистом, unsigned driver. Как у больших AC: слабо закрывается кодом; HWID бан + апелляция + карантин железа. Не обещать «DMA impossible».\n\nКласс D — социальное. Другой человек за аккаунтом, буст, стрим экрана соседу. Телефон + пароль слабые; 2FA в ЛК позже. Не путать с лицами FACE-01 (это зал).\n\nПравило фактов. Клиент шлёт наблюдения (процессы, liveliness, версия, HWID). Вердикт только в облаке. Сигнатуры чужих читов в git не класть.",
                        'path' => null,
                        'audience' => 'Система / Техник',
                    ],
                    [
                        'title' => 'Архитектура: клиент, облако, дедик',
                        'description' => "Три контура. Booking в розетки дома не ходит — клиент и плагин сами ходят HTTPS к APP_URL.\n\n1) Клиент REACTOR AC (отдельный репозиторий, не Qt-шелл). MSI/EXE. Служба Windows + UI в трее (статус, логин, версия). Kernel.sys — с фазы 4, с WHQL. Задачи службы: логин по токену ЛК, HWID, пульс, захват evidence по команде облака, не дать выгрузить себя незаметно (watchdog пары процесс+служба).\n\n2) Облако booking. Выдаёт match ticket (короткоживущий JWT или opaque token): user_id, hwid, match_id, exp. Принимает heartbeat и events. Вердикт: ok / kick / ban. Гард ArenaDuelService / Tournament: домашнему игроку connect URI отдают только если last heartbeat свежий. Плагин сервера — source of truth: даже украденный connect-пароль без ticket = kick.\n\n3) Игровой сервер клуба. Уже есть адрес в ARENA_CS2_CONNECT. Нужен плагин (CS2 server plugin / MetaMod-ветка под выбранный рантайм; Dota — отдельный контур). На connect: POST /api/ac/server/validate {ticket, steam_id, server_secret}. 403 → kick. Периодический revalidate в матче. Секрет сервера только на дедике и в .env облака, не в клиенте.\n\nИдентичность. Бан: users.id + ac_hwids[]. Домашний HWID имеет смысл (это его ПК). Смена диска/платы — новый отпечаток, детект «слишком новый HWID на старом бане» эвристикой, не железом.\n\nКлиент не банит сам. Иначе подменённый клиент скажет ok.",
                        'path' => '/admin/config/features',
                        'audience' => 'Система',
                    ],
                    [
                        'title' => 'Клиент: установщик, служба, драйвер',
                        'description' => "Дистрибутив. GET /ac/download (гость после логина ЛК) и публичная страница «Античит» по аналогии с /app.apk. Канал: EXE + SHA256 + version в /ac.json {version, url, sha256, min_os}. Автообновление службы до матча, не в раунде.\n\nУстановка. Нужны права администратора (драйвер и служба). Требования: Windows 10/11 x64, Secure Boot желателен, HVCI совместим с нашим sys. Конфликт: FACEIT AC и Vanguard на том же ПК — тестировать; при поломке Valorant/FACEIT документировать «выгрузите наш AC вне матча клуба» (как делают другие платформы).\n\nСлужба. Имя ReactorAcSvc. Старт Automatic. UI в трее: войти (device-code или refresh token ЛК), статус «готов к серверу», кнопка «проверить». Без UI матч не блокировать, если служба жива.\n\nHWID (состав, не рецепт обхода): SMBIOS UUID, серийник диска системного тома, MAC постоянного NIC, MachineGuid — хэш на клиенте, в облако уходит digest + salt версии. Не сырые серийники в публичном JSON.\n\nEvidence. По команде облака: скрин рабочего стола, список процессов (имя + sha256 образа, без дампа RAM), опционально 10 с клипа если клиент умеет. Лимит 3 МБ. Не в Telegram и не в /clips.\n\nРепозиторий. Отдельно от compclubshell (например C:\\Qt\\reactorac). Подпись Authenticode. Исходник драйвера — закрытый репо, не публичный git клуба.",
                        'path' => '/ac/download',
                        'audience' => 'Игрок / Техник',
                    ],
                    [
                        'title' => 'Игровой сервер: тикет и kick',
                        'description' => "Сейчас. ArenaDuelService::connectUri клеит ARENA_CS2_CONNECT + password. Домашний игрок из ЛК уже может получить URI. Это дыра, пока нет AC: пароль лобби = вход.\n\nЦель. Connect URI можно показать, но сервер пускает только с ticket. Два варианта (можно оба):\n• плагин читает ticket из client convar / setinfo, который выставляет клиент AC перед connect;\n• whitelist SteamID, который облако пушит на дедик (хуже: задержка, не кикает умерший агент).\nПредпочтительно ticket + revalidate каждые 30–60 с.\n\nAPI дедика (server_secret):\n• POST /api/ac/server/validate — ticket, steam_id, ip, match_id;\n• POST /api/ac/server/heartbeat — игрок ещё на сервере;\n• POST /api/ac/server/kick-ack — после кика.\nОтвет validate: allow | deny_no_ac | deny_banned | deny_stale | deny_wrong_match.\n\nЖелезо дедика. Не путать с бездиском 192.168.20.10. Отдельная машина или выделенный хост CS2, UDP игрокам из WAN. Firewall: игровой порт наружу, API только исходящий HTTPS к APP_URL. Плагин не слушает админку из интернета.\n\nЗал vs дом. Игрок с computer_id живой сессии: validate может пропустить без домашнего AC (station_trusted=true, IP из VLAN 20). Игрок с дивана — только AC. Смешанный матч (двое в зале, двое дома) допустим: у каждого свой контур.\n\nDota. Второй плагин, когда появится ARENA_DOTA_CONNECT в бою. MVP — CS2.",
                        'path' => '/admin/tournaments',
                        'audience' => 'Техник / Система',
                    ],
                    [
                        'title' => 'Фазы 0–2: оферта, клиент, gate на дедик',
                        'description' => "Фаза 0. Продукт и право (1–2 недели, без драйвера).\n• Текст оферты: домашний античит, скрины, HWID, бан железа, апелляция.\n• Регламент kick-текста на сервере.\n• Решение по стеку CS2-плагина (поддерживаемый runtime года релиза).\n• Фича reactor_ac default off: без неё сервер работает как сейчас (пароль лобби).\nПриёмка 0: страница «Скачать античит»-заглушка, оферта в /admin/config/documents.\n\nФаза 1. Служба + облако без kick (4–8 недель).\nУстановщик user-mode, логин ЛК, HWID, heartbeat POST /api/ac/heartbeat каждые 10 с, UI трея. ЛК показывает «AC: онлайн / не установлен». Сервер ещё пускает всех. Админка /admin/fair-play — лента сессий.\nТесты: AcClientSessionTest (токен, stale, бан не применяется).\nПриёмка 1: игрок ставит EXE на чистую Win11, виден в админке, матч не ломается.\n\nФаза 2. Gate (3–5 недель).\nПлагин CS2 + validate. Фича gate: домашний connect без heartbeat ≤ 20 с → kick. ЛК не показывает URI, пока AC offline (можно показать «запустите античит»). Смешанный матч: станция зала проходит station_trusted.\nПриёмка 2: без AC с дома кик за 5 с; с AC — играет; игрок с PC-08 без домашнего AC — играет; украденный пароль лобби без ticket — кик.",
                        'path' => '/admin/config/features',
                        'audience' => 'Система / Игрок',
                    ],
                    [
                        'title' => 'Фазы 3–5: баны, kernel, DMA',
                        'description' => "Фаза 3. Вердикт и HWID-бан (3–4 недели после gate).\nПравила в облаке: unknown_process в матче → kick + evidence; повтор → temp_ban 7 дней на дедик (не на бар и не на бронь ПК); perm при явном инжекте после разбора человеком. Апелляция supervisor+.\nТаблицы ac_sessions, ac_events, ac_evidence, ac_bans, ac_hwids.\nЛК: «Заблокирован на игровом сервере до ДД.MM» без инструкций обхода.\nТесты: AcBanTest, валидация дедика 403 deny_banned.\n\nФаза 4. Kernel (квартал–год, как у FACEIT обязателен для «настоящего» AC).\nНа домашнем ПК user-mode снимают. Драйвер: notify процессов, целостность службы, базовая защита от выгрузки. EV-сертификат, WHQL/аттестация, HVCI, Microsoft blocklist. BSOD у игрока дома = отзывы и отвал базы — не катить unsigned на прод.\nCoexistence: не грузить свой sys, если политика «игрок сейчас в Valorant»; либо документировать «наш AC только на время матча клуба, после матча служба может unload driver». Конфликт с FACEIT AC на одном ПК — матрица тестов до релиза.\nАльтернатива, если подпись не тянем: остаться на фазе 2–3 (gate + user-mode). Это слабее FACEIT, но уже закрывает «зашёл по IP без клиента».\nНе делать: unsigned sys «поставь тестовый», реверс чужих AC, боевые читы на стенде. Стенд — свои заглушки и VM.\n\nФаза 5. DMA. Потолок. HWID, жалобы соперника, демо/GOTV на дедике, ручной разбор. Не спринт клиента.",
                        'path' => '/admin/incidents',
                        'audience' => 'Owner / Система',
                    ],
                    [
                        'title' => 'API, таблицы, фича, дистрибутив',
                        'description' => "Фича /admin/config/features → Киберспорт → «REACTOR AC». Ключ reactor_ac. Поля: mode=off|telemetry|gate, heartbeat_stale_sec (~20), allow_station_without_ac (зал без домашнего клиента, default true), auto_kick, ban_days (7), evidence_keep_days (90). Пустой club_features не включает AC (исключение: пока контура нет). После v1 — default off, owner включает, когда дедик и плагин живы.\n\nAPI клиента (Bearer игрока, не terminal_id шелла):\n• POST /api/ac/login — обмен сессии ЛК или device-code на ac_token;\n• POST /api/ac/heartbeat — status, version, hwid, findings;\n• POST /api/ac/events — срочный факт;\n• POST /api/ac/evidence — скрин/проclist;\n• GET /api/ac/policy — версия allowlist, min_client.\nAPI дедика (server_secret): /api/ac/server/validate|heartbeat|kick-ack.\nАдминка: GET /admin/fair-play, POST /admin/fair-play/bans/{id}/pardon.\nДистрибутив: GET /ac/download, GET /ac.json.\n\nТаблицы (черновик, миграции в этом ТЗ не делать):\n• ac_sessions (user_id, hwid_hash, client_ver, last_seen_at, token_id);\n• ac_hwids (user_id, hwid_hash, first_seen, banned_at);\n• ac_events, ac_evidence, ac_bans (scope=game_server|arena|tournaments, until, reason);\n• match ticket: ac_tickets (match_id, user_id, exp).\n\nКод (целевой): репо reactorac (клиент); booking AcSessionService, AcTicketService, AcVerdictService, AcServerGuard; плагин на дедике. Не Qt-шелл. Тесты: SystemDocs (эта глава), позже AcClientSessionTest, AcServerValidateTest, AcBanTest.\n\nНе в git: дампы, сигнатуры сторонних читов, исходник драйвера до закрытого репо.",
                        'path' => '/admin/config/features',
                        'audience' => 'Система',
                    ],
                    [
                        'title' => 'ЛК, админка, сервер, зал',
                        'description' => "ЛК /account/arena и страница турнира. Баннер: скачать AC, статус «готов» / «не запущен» / «бан до …». Кнопка connect / пароль сервера серые, пока gate и AC offline. Казуальная бронь ПК и бар не зависят от AC.\n\nСайт. Публичный блок «Играй с дома» + требования Win10/11, размер установщика, что античит нужен только на сервер клуба.\n\n/admin/fair-play. Сессии домашних клиентов, HWID, события, скрины, кики с дедика, помиловать. Не плитки зала.\n\nДашборд зала. Не смешивать с flap/drift. Отдельный счётчик «домашних на сервере: N».\n\nШелл. Не ставит REACTOR AC и не шлёт /api/ac/*. Trusted station — побочный путь validate, не агент в Main.cpp.\n\nTV. Не показывает античит.\n\nЛожные кики. Цель фазы 2: дом без AC всегда кик (это не ложный); дом с AC <1% ложных kick. Allowlist пополняется ревизией политики (Discord overlay и т.п.), не админом смены «на глаз».",
                        'path' => '/account/arena',
                        'audience' => 'Игрок / Supervisor+',
                    ],
                    [
                        'title' => 'Право, подпись, конфликт с чужими AC',
                        'description' => "152-ФЗ. HWID, список процессов, скрин — данные на домашнем ПК. Основание: оферта соревнования на сервере клуба, добровольная установка. Без установки на дедик не пускаем — это условие услуги, не скрытый агент. Срок хранения evidence, доступ supervisor+/owner, не публикация, удаление после апелляции. Кейлог нажатий не писать.\n\nMicrosoft. Драйвер без EV/WHQL на Win11 с HVCI не жилец. Тестовый sys игрокам не раздавать.\n\nЧужие античиты. На домашнем ПК уже могут стоять FACEIT AC, Vanguard, EAC. Запрещено их патчить и эмулировать. Регресс: CS2+VAC, CS2+FACEIT, Valorant, Dota. Если наш kernel валит VAN — unload или не грузить, иначе игроки не поставят клиент.\n\nУголовка. Боевые читы не держать «для теста» на проде. Стенд: VM и свои заглушки.\n\nОтветственность. AC снижает читов на своём дедике, не делает клуб FACEIT. Спорный кик — рефери force-refund / переигровка, как сейчас арена.",
                        'path' => '/admin/config/documents',
                        'audience' => 'Owner / Юрист',
                    ],
                    [
                        'title' => 'Дорожная карта, оценка, приёмка',
                        'description' => "Порядок. 0 оферта и страница загрузки → 1 служба+heartbeat → 2 плагин дедика и kick → 3 HWID-бан и evidence → 4 kernel с подписью → 5 DMA как операционка.\nНе начинать с драйвера и не встраивать агент в Qt-шелл.\n\nОценка. Фаза 0: 1–2 нед. Фаза 1: 4–8 нед. (отдельный клиент). Фаза 2: 3–5 нед. (плагин CS2 + API). Фаза 3: 3–4 нед. MVP «с дома без AC не зайти» ≈ 3–4 месяца. Фаза 4: отдельный бюджет и год на подпись/совместимость, если цель «как FACEIT по силе».\n\nMVP. Установщик user-mode, логин ЛК, heartbeat, плагин кикает без ticket, зал без клиента проходит, фича gate на одном дедике CS2. Автобан можно не включать первую неделю.\n\nЧеклист приёмки MVP.\n• Фича off — сервер как сейчас, пароль лобби.\n• telemetry — виден в fair-play, кика нет.\n• gate — дома без AC kick; дома с AC играет; PC-08 без домашнего AC играет.\n• /ac/download отдаёт EXE, sha256 сходится.\n• Evidence не в /clips и не в Telegram.\n• Бан дедика не закрывает бар и бронь ПК.\n• Valorant на том же домашнем ПК после выхода из нашего матча запускается (матрица без нашего kernel на MVP).\n\nНе успех. Агент в шелле зала. Бан HWID PC-08. Unsigned драйвер игрокам. Сигнатуры читов в публичном репо.\n\nСвязанные главы. Арена (connect URI), турниры, ЛК, оферта. Шелл и киоск — не этот контур.",
                        'path' => '/admin/system-tests',
                        'audience' => 'Owner / Система',
                    ],
                ],
