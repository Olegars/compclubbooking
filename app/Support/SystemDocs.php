            ...
            [
                ...
                'items' => [
                    ...
                    [
                        ...
                        'description' => "Редактирование никнейма, email и фото (ЛК /account/profile и карточка аватара в кабинете). Телефон зафиксирован как идентификатор входа. Стартовый ник при первой регистрации выдаёт DeepSeek (см. «Игровой ник»).\n\nСвоё фото: POST /account/profile/avatar. Галка «Стилизовать под клубный формат». DeepSeek картинки не рисует (только текст).\n\nПорядок: 1) лицо с фото в овал стандартного avatar_*.png (броня и схемы шаблона остаются); 2) если задан COMFYUI_URL — ComfyUI (img2img композита; при графе с двумя LoadImage — фото + образец, PuLID/IP-Adapter FaceID); 3) иначе Hugging Face Inference Providers (HF_TOKEN, модель Qwen/Qwen-Image-Edit) дорисовывает композит; 4) иначе OpenAI images/edits, если ключ есть; 5) иначе остаётся локальный композит.\n\n127.0.0.1:8188 работает только если ComfyUI на той же машине, что PHP. Booking в облаке до клубного GPU сам не достучится — нужен публичный URL, туннель или агент. HF: не безлимитный хаб из тысяч моделей, а Inference Providers с кредитами/лимитами; FaceID там обычно нет.",
