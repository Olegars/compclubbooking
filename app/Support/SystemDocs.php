        ...
        return [
            ...
            [
                ...
                'items' => [
                    ...
                    [
                        ...
                        'description' => "Клубный античит для арены, турниров с призами, Clan Wars и охоты — по архитектуре как FACEIT AC (user-mode + облачный вердикт + Trusted Mode до матча), но на своих ПК, а не на домашнем железе гостя. Бан вешается на игрока (телефон / user_id), не на HWID станции. Не замена VAC / EAC / Vanguard / FACEIT AC: они остаются в образе. Полный план — раздел «Античит клуба (REACTOR AC)».",
                        'path' => '/admin/docs',
                        'audience' => 'Supervisor+ / Техник / Система',
                    ],
                ],
            ],
            [
                'id' => 'anticheat',
                'title' => 'Античит клуба (REACTOR AC)',
                'items' => [
                    [
                        'title' => 'Назначение: честная конкуренция в зале',
                        'description' => "Зачем. В клубе уже есть деньги и репутация в киберспорте: охота (эскроу депозита), арена (Elo, босс клуба, перк царя горы), турниры с призами на депозит, Clan Wars. Читер на чужом ПК ломает не «свой аккаунт», а чужой вечер и кассу заведения. Нужен контур честности своего соревновательного слоя — как FACEIT AC на платформе FACEIT, только для REACTOR.\n\nЧто защищаем (приоритет):\n1) Матчи с выплатой или призом: охота, турнир, перк арены.\n2) Рейтинг: Elo арены, вклад Clan Wars, трон ПК.\n3) Репутация клуба, если гость играет FACEIT/Премьер с клубного ПК (не подменяем их античит, не мешаем ему).\n\nЧто не цель. Казуальная сессия без рейтинга: гость купил час CS2 — не срываем игру ложным киком. Там телеметрия и тикет админу, не автобан. Не цель — ловить читы «вообще в Windows» на домашнем ПК игрока: машины клуба, образ один, writeback после reboot выкидывается.\n\nНе путать. Киоск SecurityManager — чтобы гость не ушёл в TaskMgr. Golden Image Drift — порча D:, не аимбот. HID / Hardware Health / Rage-Smash — периферия и удар по столу. REACTOR AC — честность матча.\n\nСтатус. ТЗ / план разработки. В коде шелла и booking контура ещё нет (нет /api/shell/ac/*, нет фичи reactor_ac, нет банов ac_ban). Ниже — слои, фазы, API, право и приёмка, чтобы писать по главам, а не «сделать античит».",
                        'path' => '/admin/docs',
                        'audience' => 'Owner / Supervisor+ / Техник',
                    ],
                    [
                        'title' => 'Как у FACEIT — и чем клубной античит другой',
                        'description' => "Публичная схема FACEIT AC (ACE), на которую ровняемся по слоям, не копируя бинари и не реверся драйвер:\n• user-mode агент, который должен быть жив до старта матча;\n• kernel-драйвер на домашнем ПК (враждебная машина игрока);\n• облако: сердцебиение, сигналы, вердикт, бан аккаунта + отпечаток железа;\n• Trusted / anti-cheat must be running — иначе в матч не пускают;\n• доказательства (скрин, снимок процессов) для апелляции;\n• постоянный бан платформы.\n\nКлуб — другая машина угроз.\n• ПК наши: золотой образ CCBoot, киоск, нет прав админа у гостя, C: writeback умирает после reboot. Домашний FACEIT AC рассчитан на то, что игрок root у себя. У нас root у техника.\n• HWID станции общий на 40 мест и на всех гостей смены. Бан «железа» как у FACEIT здесь бессмыслен: забанишь PC-08 — сядет следующий. Бан только users.id + phone. Карантин станции — отдельный тикет технику (подозрение на DMA/прошивку мыши), ПК выводится из пула арены/турнира, не «бан клуба».\n• Сессия конечная: чит, живущий только в RAM/writeback, сдохнет на reboot. Это не повод не ловить его в матче — приз уже выплачен.\n• D: кэш игр живёт между ребутами. Подмена бинарника CS2 на D: — и drift, и AC.\n• Несколько античитов игр уже в образе (EAC, BattlEye, vgk). Второй kernel-драйвер клуба легко поссорится с Vanguard (VAN 128/157) и с FACEIT AC, если гость зайдёт на FACEIT с этого ПК. Поэтому kernel — фаза 4, опция, не старт.\n\nВывод. Берём у FACEIT продуктовую дугу: агент → обязателен до соревновательного матча → облачный вердикт → бан личности → доказательства. Не берём: бан HWID ПК, обязательный свой ring-0 в v1, охоту за читами на домашних машинах.",
                        'path' => null,
                        'audience' => 'Owner / Техник',
                    ],
                    [
                        'title' => 'Модель угроз на гибридном бездиске',
                        'description' => "Контур: C: образ + writeback, D: SSD-кэш, гость без админа, киоск, Super Client только у техника. Booking в розетки не ходит.\n\nКласс A — сессия, user-mode (закрываем в фазах 1–2).\n• Запуск постороннего процесса поверх игры (оверлей ESP, лоадер, «панель»).\n• Инжект модуля в процесс игры.\n• Подмена/патч exe и dll игры на D: (Steam common, EasyAntiCheat-каталог как индикатор, не как свой движок).\n• USB-флешка с лоадером в Trusted Mode.\n• Макросы софта (AHK и аналоги) — синтетический ввод; LL-хуки шелла уже отличают INJECTED (Hardware Health). Прошивка мыши с макросом на борту — класс C.\n• Чтение чужого GSI/сетевого сниффера в VLAN 20 (радар с соседнего места). Изоляция по местам — не AC, а RB5009; AC может только заметить второй процесс-сниффер.\n\nКласс B — обход шелла (фаза 1 + киоск).\n• Убить REACTOR и играть из explorer. Уже ломает киоск и heartbeat; AC добавляет: нет живого агента → нет допуска в арену/турнир, тикет ac_agent_dead.\n• Загрузка с USB, если в BIOS не закрыт PXE-only. Операционка, не код AC: UEFI пароль, Secure Boot, PXE first. В чеклисте фазы 0.\n\nКласс C — железо, переживает reboot (фазы 4–5 и регламент зала).\n• DMA-плата в PCIe (чтение памяти игры с соседнего устройства). Writeback не спасает. Детект на уровне клуба почти всегда операционный: пломба корпуса, фото слотов, камера, запрет «своей периферии» в Trusted Mode.\n• Прошивка мыши/клавиатуры с аим-ассистом. Карантин девайса (HID serial), не бан ПК навсегда.\n• Подмена GPU-прошивки / unsigned driver. Золотой образ + HVCI + список драйверов vs эталон ревизии.\n\nКласс D — социальное.\n• Другой человек за бронированным местом (смёрф рейтинга). Биометрия FACE-01 в ТЗ лиц — опциональный якорь «кто сел», не замена AC.\n• Сговор двух ПК (стрим экрана). Камеры зала + Trusted Mode без Discord overlay; не детект в драйвере.\n\nЛожные цели. Не писать сигнатуры конкретных читов в git и не тащить «детект как у чита X». Облако держит правила вердикта; клиент шлёт факты (процессы, хэши, liveliness, аномалия GSI). Иначе античит устаревает в день публикации образа.",
                        'path' => null,
                        'audience' => 'Техник / Система',
                    ],
                    [
                        'title' => 'Архитектура слоёв (как ACE, под REACTOR)',
                        'description' => "Пять слоёв. Слой 4 не нужен, чтобы запустить продукт.\n\nСлой 0 — гигиена образа (уже частично есть). Киоск, production=true, PXE-only, античиты игр в Program Files на C:, игры на D:, Super Client только у техника, Rollback Markers, GoldenImageDriftWatchdog. AC не заменяет это.\n\nСлой 1 — ReactorAcService в шелле (user-mode, процесс шелла или дочерний сервис под той же сессией). Стартует с Main.cpp рядом с GoldenImageDriftWatchdog. Не отдельный «античит. exe» в автозагрузке Windows: киоск и так Shell=REACTOR. Задачи: allowlist процессов во время игры, инвентарь модулей игрового PID (ProcessManager уже знает cs2/EAC stub), USB-события, liveliness-токен, скрин по запросу облака, heartbeat в том же POST /api/shell/power/heartbeat (поле ac) плюс срочный POST /api/shell/ac/events.\n\nСлой 2 — облако booking (вердикт). AcTelemetryService принимает факты, не «чит/не чит» с клиента. Правила в AcPolicy (YAML/JSON в club_features.reactor_ac или таблица ac_rules): пороги, allowlist, что в Trusted Mode запрещено. Вердикт: ok / warn / kick_match / ban_pending / ban. Клиент не имеет права сам бандить — иначе лоадер подменит агент.\n\nСлой 3 — идентичность и санкции. ac_enforcement на user_id (+ club_id). Типы: warn, match_forfeit, temp_ban (сутки/неделя), perm_ban на арену/турниры (казино зала можно оставить — решение owner). Не банить вход в клуб и бар по умолчанию: это не FACEIT-аккаунт, это гость заведения. Флаг users.ac_restricted_until / таблица ac_bans. Апелляция: supervisor+ смотрит пакет доказательств и снимает.\n\nСлой 4 — kernel (опция). Мини-фильтр / notify-процессов / колбэки объекта, подпись WHQL, совместимость HVCI. Только если слой 1 массово обходят в зале и юрист/Microsoft-подпись закрыты. Не грузить, когда в сессии Valorant (vgk) или гость явно запускает FACEIT AC — политика coexistence, иначе сломаем платящих гостей.\n\nСлой 5 — зал. Камера + метка NVR по событию ac.violation (как hardware.abuse). Trusted Mode: USB mass storage policy, запрет своей клавиатуры с макросами (HID allowlist клубных моделей — опция). Пломба PCIe.\n\nПоток матча (арена / турнир / охота). 1) Шелл в match-context (GSI in_match или ArenaDuel live). 2) Облако требует ac.status=healthy из последнего heartbeat ≤ N с. 3) Нет агента / drift критичный / USB в Trusted — старт дуэли 422, турнирный слот не открывает игру (поверх lock_games). 4) Во время матча событие severity≥kick → forfeit, оппоненту победа (как дисконнект 90 с арены). 5) Пакет доказательств в ac_evidence, тикет /admin/incidents type=ac_violation.",
                        'path' => '/admin/config/features',
                        'audience' => 'Система / Shell',
                    ],
                    [
                        'title' => 'Что уже есть и куда встраивать',
                        'description' => "Не писать второй шелл. Точки входа:\n\nШелл (C:\\Qt\\compclubshell).\n• main.cpp — рядом с GoldenImageDriftWatchdog / HardwareHealthWatchdog создать ReactorAcWatchdog.\n• ProcessManager — список процессов и «игра vs EAC stub vs splash»; AC читает тот же PID, не дублирует EnumProcesses без нужды.\n• SecurityManager — киоск; AC на idle почти молчит (только liveliness), в сессии — allowlist.\n• NetworkManager heartbeat — уже тащит integrity_*; добавить блок ac {status, revision, nonce, findings[]}.\n• GoldenImageDriftWatchdog — хэши D:; критичный drift игры в Trusted = не стартовать матч, не только плитка «drift D:».\n• HardwareHealthWatchdog — флаг INJECTED; AC может подписаться: пачка синтетики во время арены → finding input_injected (warn, не бан с первого раза).\n• Instant Replay — буфер 60 с на D:/ShellData/replay: по команде evidence с облака кропнуть 15 с в ac_evidence, не в кабинет гостя.\n• GSI POST /api/shell/gsi — аномалии (невозможный счётчик, резкий hop рейтинга) считает облако, не шелл.\n\nBooking (C:\\Qt\\compclubbooking).\n• ShellIncidentService — новые типы ac_violation, ac_agent_dead (не сваливать в golden_image_drift).\n• ArenaDuelService / LanBounty / TournamentService / ClanWarService — гарды: игрок не в ac_bans, ПК не в quarantine, агент healthy.\n• ClubFeatureCatalog group=esports ключ reactor_ac (тумблер, режим casual_telemetry | competitive_gate, TTL heartbeat, auto_forfeit).\n• ComputerPowerService heartbeat extras — как integrity_*.\n• VideoMarkerService — триггер ac.violation, метка как Rage-Smash.\n• /admin/incidents + новая /admin/fair-play (лента вердиктов, доказательства, апелляция) — owner/supervisor.\n\nОбраз. Каталог allowlist: шелл, Steam, игра, EAC/BE/Vanguard, NVIDIA overlay выкл в Trusted, Discord overlay выкл в Trusted. Хранить эталон allowlist в ревизии Rollback Marker или отдельном ac_allowlist.json на C: (в образе), подпись хэшем, который облако знает.\n\nНе в AC. Вентиляторы, DMX, магазин, SMS, лица FACE-01 (якорь «кто сел» — соседний контур).",
                        'path' => '/admin/incidents',
                        'audience' => 'Техник / Shell / Система',
                    ],
                    [
                        'title' => 'Фазы 0–2: гигиена, агент, Trusted Mode',
                        'description' => "Фаза 0. Регламент и гигиена (без нового бинаря, 1–2 недели зала + доки).\n• BIOS: PXE first, пароль UEFI, отключить USB-boot, Secure Boot on.\n• Образ: [Security] production=true, киоск проверен на 2 ПК, Faceit/EAC/Vanguard стоят и запускаются (регресс).\n• USB: в зале ящик «своя периферия»; для турнира — только клубные мыши/клавы (список в доке, не в драйвере).\n• PCIe: пломба корпуса, фото слотов в карточке ПК (можно позже Digital Twin).\n• Политика санкций на /admin/config/documents: что считается нарушением, сроки бана, что апелляция. Гость видит короткий текст в Trusted Mode.\n• Фича reactor_ac=off по умолчанию, пока агента нет.\nПриёмка 0: чеклист техника на 5 ПК; гость без админа; USB-boot не грузит Windows.\n\nФаза 1. User-mode агент + телеметрия (4–6 недель).\nШелл: ReactorAcWatchdog. Пульс раз в 5–10 с в heartbeat. Поля: status=ok|degraded|dead, agent_rev, os_secure_boot, usb_mass_storage, game_pid, findings[{code,severity,detail}]. Коды фактов (не вердикт): unknown_process, overlay_window, usb_inserted, integrity_game_drift, agent_tamper, input_injected, driver_not_in_baseline (если собрали список).\nAllowlist: config.ini [AntiCheat] плюс облако (features) может дописать. Игровые античиты и шелл всегда разрешены.\nСобытия срочные: POST /api/shell/ac/events {terminal_id, code, severity, payload} — не ждать 30 с heartbeat, если unknown_process в live-дуэли.\nОблако: пишет ac_events, не банит. Дашборд: точка на плитке (зелёная/жёлтая). Инцидент ac_agent_dead если heartbeat без блока ac ≥ 2 мин при активной сессии и фича on.\nТесты: ShellAcTelemetryTest — heartbeat с ac.status, событие, фича off → шелл не шлёт / сервер игнор.\nПриёмка 1: казуал не кикает; админ видит процесс-нарушитель в ленте; ложный тикет на EAC splash нет (ProcessManager stub уже фильтрует).\n\nФаза 2. Trusted Mode — допуск в соревнование (3–4 недели).\nКонтексты: арена live, турнир lock_games, охота эскроу, опционально Clan Wars. GSI in_match + флаг trusted из облака.\nГарды старта: POST /api/shell/arena/challenges /accept /start и bounty create → 422 ac_unhealthy / ac_banned / pc_quarantine. Шелл не стартует exe турнира, пока login/games не получили ac_gate=ok.\nВо время матча: kick_match → ArenaDuelService техническое поражение (тот же путь, что дисконнект 90 с), охота — отмена эскроу в пользу оппонента по политике (зафиксировать в фазе 0: читер не забирает банк).\nUSB mass storage в trusted → finding, по политике kick или warn.\nОверлей Discord/NVIDIA в trusted — allowlist выкл, finding overlay_window.\nПриёмка 2: без агента дуэль не стартует; убитый шелл в матче = forfeit; казуальный CS2 без арены идёт; Valorant/Vanguard не падают.",
                        'path' => '/admin/config/features',
                        'audience' => 'Техник / Shell / Supervisor+',
                    ],
                    [
                        'title' => 'Фазы 3–5: вердикт, kernel, DMA',
                        'description' => "Фаза 3. Вердикт, доказательства, баны (3–4 недели после фазы 2).\nAcVerdictService: правила на сервере. Пример политики (не код детекта): 1× unknown_process в казуале = warn в инциденты; 1× в trusted = kick_match + пакет evidence; 2 kick за 30 дней = temp_ban арены 7 дней; integrity_game_drift критичный = не стартовать + resync как сейчас, бан только если повторно после чистого образа.\nДоказательства ac_evidence: снимок findings, список процессов (имена/хэши, без полной RAM), 15 с replay если был буфер, скрин рабочего стола по запросу GET в ответе heartbeat ac_command=capture (шелл кладёт PNG на POST /api/shell/ac/evidence, лимит ~3 МБ). TTL хранения 90 дней, доступ supervisor+/owner, не в ЛК гостя.\nАпелляция: /admin/fair-play кнопка «Снять бан». Аудит: кто снял.\nЛК игрока: «Ограничен в арене до ДД.MM» без технических деталей (не учить обходу).\nМетка NVR ac.violation.\nКарантин ПК: computers.ac_quarantine_until — DMA/подозрение на прошивку; дашборд плитка; не матчить в турнир.\nТесты: AcBanTest, ArenaDuelsTest (старт 422), Tournament lock + ac_banned не в сетке.\n\nФаза 4. Kernel-агент (квартал–год, отдельное решение owner).\nЗачем как у FACEIT: user-mode можно убить, если гость снял киоск или нашёл дыру. Ring-0 видит создание процесса раньше.\nСтоимость: EV-сертификат + WHQL/аттестация, HVCI, блоклист уязвимых драйверов Microsoft, совместимость с vgk / FACEIT AC / EAC, BSOD = простой зала, Rollback Marker не спасёт чужой sys. Юрист: согласие на мониторинг в оферте.\nПравило coexistence: не грузить reactorac.sys если запущен vgk или faceit клиент (детект по сервису, не по хукам чужого AC). Иначе лучше не иметь слой 4.\nАльтернатива слою 4: жёстче киоск + watchdog сервиса шелла (рестарт процесса) + Trusted Mode без своего драйвера. Для клуба с образом это часто достаточный 80%.\nНе делать: unsigned sys «для теста на зале», маппинг чужих античитов, поставка эксплойтов «для проверки». Проверка — свои фикстуры (процесс-заглушка not-a-cheat.exe в allowlist тестов), не боевые читы.\n\nФаза 5. DMA и прошивки (потолок, как у больших AC).\nПрограммно закрывается слабо. Операционка: пломба, запрет вскрытия, камера, USB-HID allowlist, периодический осмотр слотов, карантин ПК при жалобе соперника + совпадение «идеальные флаги» GSI. Внешние античиты игр сами борются с DMA; клуб не обещает «DMA impossible».\nВ план разработки зала, не в спринт шелла.",
                        'path' => '/admin/incidents',
                        'audience' => 'Owner / Supervisor+ / Техник',
                    ],
                    [
                        'title' => 'API, таблицы, фича',
                        'description' => "Фича /admin/config/features → Киберспорт → «REACTOR AC». Ключ reactor_ac. Выкл: шелл не шлёт ac, гарды арены/охоты/турнира не смотрят агента. Поля: mode=telemetry|gate (gate = Trusted Mode), heartbeat_stale_sec (~20), auto_forfeit, usb_block_in_trusted, overlay_block_in_trusted, ban_arena_days (7), evidence_keep_days (90). Пустой club_features не включает AC: здесь исключение из правила «пусто = всё вкл», пока контура нет. После релиза v1 — явное default off, owner включает на клубе.\n\nAPI шелла (все с terminal_id, сессия как остальные /api/shell/*):\n• блок ac в POST /api/shell/power/heartbeat и ответ ac_command: none|capture|refresh_policy;\n• POST /api/shell/ac/events — срочный факт;\n• POST /api/shell/ac/evidence — multipart PNG/mp4 + event_id;\n• GET /api/shell/ac/policy — allowlist и режим (можно внутри login/balance, чтобы не плодить poll).\nГарды: ArenaDuelService, bounty, TournamentService::register / lock games — AcGate::assertPlayer(user), assertStation(computer).\nАдминка: GET /admin/fair-play, POST .../bans/{id}/pardon (supervisor+). Инциденты type=ac_violation | ac_agent_dead.\n\nТаблицы (черновик миграций, не создавать в этом ТЗ):\n• ac_events (club_id, computer_id, user_id, booking_id, match_kind, code, severity, payload json, created_at);\n• ac_evidence (event_id, kind=screenshot|replay|proclist, path/disk, sha256, bytes);\n• ac_bans (user_id, club_id, scope=arena|tournaments|bounty|all_comp, until, reason, evidence_event_id, pardoned_at, admin_id);\n• computers.ac_quarantine_until, computers.ac_status;\n• users — не плодить колонку, если хватает ac_bans.\n\nШелл config.ini [AntiCheat] enabled, pulse_ms, capture_max_mb. Код (целевой): ReactorAcWatchdog, AcTelemetryService, AcVerdictService, AcGate, ClubFeatureCatalog::reactor_ac. Тесты: SystemDocs (эта глава), позже ShellAcTelemetryTest, AcBanTest, AcGateArenaTest.\n\nНе класть в git: дампы памяти, сигнатуры сторонних читов, исходники драйвера до отдельного закрытого репо с подписью.",
                        'path' => '/admin/config/features',
                        'audience' => 'Система',
                    ],
                    [
                        'title' => 'Админка, шелл, гость',
                        'description' => "Дашборд. Плитка: точка AC (ok / stale / quarantine). Карточка ПК: ac.status, last finding, кнопка «Карантин / снять» (owner). Не смешивать с flap и drift — отдельная строка.\n\n/admin/fair-play (Киберспорт). Лента событий, фильтр по игроку/ПК/матчу, превью скрина, «Форфейт», «Бан арены 7д», «Помиловать». Связь с инцидентом.\n\nШелл. Казуал: тихо. Trusted: узкая плашка «Честная игра · AC ok» (не оверлей поверх прицела — под сайдбаром). При kick: SessionAlert + TTS, возврат в лобби арены. При бане: «Арена недоступна до …» на вкладке АРЕНА, охота серая.\n\nЛК. Если бан scope соревновательный — баннер в /account/arena и на карточках вызова. Бар, бронь ПК, магазин — без ограничений по умолчанию.\n\nTV. Не показывает факты AC (это не счёт Clan Wars).\n\nОферта. Короткий абзац: в соревновательных режимах клуб пишет служебные снимки процессов и короткий клип/скрин для разбора нарушений; хранение N дней; не для публикации в Reels. Галка оферты уже есть (offer_accepted_at) — дополнить текст /admin/config/documents, не отдельный SDK.\n\nЛожные срабатывания. Владелец смотрит долю kick / матч. Цель фазы 2: <1% trusted-матчей с ложным forfeit. Allowlist пополняется с зала (Discord overlay, RGB-софт вендора мыши) только через ревизию политики, не «разрешил админ смены на одном ПК».",
                        'path' => '/admin/docs',
                        'audience' => 'Админ / Игрок / Supervisor+',
                    ],
                    [
                        'title' => 'Право, приватность, конфликт античитов игр',
                        'description' => "152-ФЗ. Скрин рабочего стола и список процессов — персональные данные сессии. Правовое основание: договор оказания услуг + оферта, цель — пресечение нарушений регламента соревнования. Не выкладывать evidence в интернет, не слать в Telegram-канал клипов, доступ роли supervisor+/owner, срок хранения, удаление по запросу после апелляции. Не писать кейлог (нажатия WASD) в AC — для читов не нужно, для закона вредно. Hardware Health хуки уже есть для микрика; AC не расширяет их до записи клавиш.\n\nТрудовой/гость. Это не контроль сотрудников. Админский ПК и FACE-01 в контур не входят.\n\nИгры. VAC/EAC/Vanguard/FACEIT AC — чужие продукты. Запрещено: хукать их, патчить, выгружать, эмулировать «я FACEIT AC». Разрешено: не мешать, allowlist их процессов, при конфликте своего kernel — не грузить свой. Регресс-матрица фазы 1 и 2: CS2 (VAC + опционально FACEIT), Valorant (Vanguard), Dota, Fortnite EAC, Riot League (Vanguard-adjacent). Сломанный VAN на образе = откат, не «починим античитом клуба».\n\nMicrosoft. Свой sys без подписи на Win11 + HVCI не взлетит или попадёт в blocklist. Не ставить тестовый драйвер на золотой образ зала.\n\nУголовка. Не хранить и не запускать боевые читы «для теста» на клубных ПК. Тестовый стенд: свои заглушки, VM, не продакшен-образ.\n\nОтветственность зала. AC снижает, не обнуляет риск. Приз турнира при спорном вердикте — регламент: рефери (supervisor) может отменить матч вручную, как сейчас force-refund арены.",
                        'path' => '/admin/config/documents',
                        'audience' => 'Owner / Юрист / Техник',
                    ],
                    [
                        'title' => 'Дорожная карта, оценка, приёмка',
                        'description' => "Порядок. Не начинать с драйвера. 0 гигиена → 1 телеметрия → 2 gate на арену/охоту/турнир → 3 баны и evidence → решение по 4 kernel → 5 регламент DMA.\n\nОценка (один разработчик шелл+Laravel, без WHQL): фаза 0 — 1–2 нед.; 1 — 4–6 нед.; 2 — 3–4 нед.; 3 — 3–4 нед. Итого MVP честной арены ≈ 3 месяца календарных. Фаза 4 — отдельный бюджет (подпись, BSOD-риск, совместимость). Фаза 5 — чеклист техника, не спринт.\n\nMVP (можно включать reactor_ac=gate на одном клубе). Агент в шелле; heartbeat ac; Trusted на арене и охоте; forfeit при мёртвом агенте и unknown_process; инцидент; без автобана на первую неделю (только warn+forfeit, бан руками supervisor). Турнир — второй заход, когда ложных forfeit мало.\n\nМетрики. Доля trusted-матчей с finding; доля ложных (pardon/сутки); время от события до тикета; регресс: Valorant login, FACEIT клиент, EAC старт, drift D: как раньше, киоск на месте.\n\nЧеклист приёмки MVP.\n• Фича off — ни одного поля ac, арена как сейчас.\n• Фича telemetry — факты в ленте, матч не кикает.\n• Фича gate — дуэль без агента не стартует; казуал без арены стартует.\n• EAC splash не finding.\n• USB в казуале — warn или тишина (политика); в trusted — kick по флагу usb_block_in_trusted.\n• Убить процесс шелла в дуэли — forfeit оппоненту, не зависание эскроу охоты.\n• Скрин evidence не попадает в /clips и Telegram.\n• Бан арены не закрывает бар и бронь.\n• System-tests: предупреждение, если quarantine или доля stale агентов > порога (добавить контур, не ломать текущие кнопки).\n\nНе успех. «Написали драйвер как FACEIT» без подписи и с падающим Vanguard. «Сигнатуры 50 читов в образе». Бан HWID PC-08.\n\nСвязанные главы. Shell: киоск, Super Client, Rollback Markers, drift, Hardware Health, Instant Replay, арена, охота. Киберспорт: турниры, Clan Wars. Сеть: VLAN, ПК лиц не в контуре. Безопасность: инциденты. Оферта: документы устройства.",
                        'path' => '/admin/system-tests',
            ...
            [
                ...
                'items' => [
                    ...
                    [
                        'title' => 'ПК лиц',
                        'description' => 'Служебный GPU-ПК в зале: метки NVR, распознавание лиц по субпотокам, ComfyUI для клубного аватара. ТЗ — раздел «Сеть клуба»: главы «ПК лиц». Не бездиск .10 и не игровое место. Облако на :8188 не ходит — только HTTPS наружу, как у агента меток.',
                        'path' => '/admin/docs',
