            ...
            [
                ...
                'items' => [
                    ...
                    [
                        ...
                        'description' => "Железо. Клиент и kernel FACEIT AC живут на C: золотого образа (Super Client, диск image). Не ставить AC в writeback гостя: после reboot пропадёт, на 40 ПК поедет разный билд, HWID-сервис сломается. Обновление клиента/AC — тем же Super Client, что шелл и VC++. Регресс: после save образ грузится на втором ПК, FACEIT AC-сервис в services.msc Running до логина гостя.\n\nПрофиль. %LOCALAPPDATA%\\FACEIT и данные AC-клиента на C: — writeback, выкидывается. Цель: junction/symlink на D:/ShellData/faceit/{user_id} в login шелла, снять на logout. Иначе гость каждый раз логинится заново либо оставляет сессию следующему. Не класть FACEIT на C:\\Users\\user\\AppData как в текущем exe_path сида — путь завязан на учётку образа; править в /admin/quick-apps под реальный профиль и D:.\n\nLogout / idle shutdown. Убить FACEIT.exe и UI-процессы клиента. Службу FACEIT AC не останавливать (системная, в образе). Стереть D:/ShellData/faceit/{user_id} (сессия, куки), не общий инсталл. Следующий гость не должен быть залогинен чужим ником.\n\nКонфликт с REACTOR AC. На клубном ПК гость в FACEIT MM использует FACEIT AC; REACTOR AC на зал не ставится (шелл = station_trusted на дедик клуба). На домашнем ПК оба клиента не патчить друг друга — глава античита «Право, подпись, конфликт с чужими AC».\n\nКиоск. FACEIT и AC — доверенные в SecurityManager (как Steam/Discord). Не Disable для их служб. Win+R по-прежнему закрыт.",
                        'path' => null,
                        'audience' => 'Техник / Shell',
                    ],
                    [
                        'title' => 'FACEIT Connect: привязка аккаунта',
                        'description' => "Как «Привязать Telegram», только OAuth2 FACEIT Connect 3.0 (authorization code, секрет на сервере). Не ник руками и не скрейп HTML.\n\nПоток.\n1) ЛК /account/profile — «Привязать FACEIT». GET /account/faceit/redirect → authorize (openid_configuration https://api.faceit.com/auth/v1/openid_configuration). state в сессии, PKCE если клиент это требует в App Studio.\n2) Callback GET /account/faceit/callback?code=&state=. Облако меняет code на access + refresh + id_token (client_secret только .env).\n3) Userinfo / Data API: player_id, nickname, avatar, steam_id_64, games.cs2.skill_level, faceit_elo, game_player_id.\n4) Пишем faceit_identities (unique faceit_player_id, unique user_id). Чужой клубный аккаунт с тем же player_id — отказ, как у telegram_chat_id.\n5) Unlink POST /account/faceit/unlink: строки нет, refresh не хранить «на всякий».\n\nScopes — минимальные из App Studio для профиля и чтения статистики, не чат-бот. Redirect URI один на приложение (ограничение FACEIT Connect): https://{APP_HOST}/account/faceit/callback. Несколько доменов = несколько OAuth-клиентов или один канонический хост.\n\nШелл OAuth не открывает: киоск съест попап FACEIT. Привязка с телефона в ЛК, на ПК только статус.",
                        'path' => '/account/profile',
